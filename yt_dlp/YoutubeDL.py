...
class YoutubeDL:
        ...
        for f in formats:
            ...
            if working is not None:
                if working:
                    yield f
                continue
            if (probe_available and ((f.get('video_ext', 'none') != 'none'
                                      and (f.get('height') is None or f.get('fps') is None))
                                     or ((f.get('audio_ext', 'none') != 'none' or f.get('acodec') != 'none')
                                         and f.get('asr') is None))):
                self.to_screen('[info] Probing format {}'.format(f['format_id']))
                if self.params.get('verbose', False):
                    ffmpeg._downloader = self
                self._before_fd_download(f, False, True)
                if data := ffmpeg.get_metadata_object(f['url'], ['-timeout', '5000000']):
                    f = merge_dicts(f, traverse_obj(data.get('format'), {
                    ...
                    for stream in traverse_obj(data, 'streams', expected_type=list):
                        ...
                        if stream.get('codec_type') == 'video':
                            ...
                            f['resolution'] = (f.get('resolution')
                                               or f"{f['width']}x{f['height']}" if f.get('width') and f.get('height') else None)
                    ...
                    f['tbr'] = f.get('tbr') or ((f.get('vbr') or 0) + (f.get('abr') or 0))
                    f['__working'] = True
    ...
    def _before_download(self, info, subtitle=False, test=False):
        """ Run extractor's _before_download before invoking file downloader """
        ie_key = info.get('extractor_key')
        if ie := traverse_obj(
                # get the current IE or the IE specified in info
                dict(reversed(self._ies_instances.items())), (..., {object}),
                default=self.get_info_extractor(ie_key) if ie_key else None,
                get_all=False):
            if callable(getattr(ie, '_before_download', None)):
                info = ie._before_download(info, subtitle, test) or info
    ...
    def dl(self, name, info, subtitle=False, test=False):
        ...
        self._before_download(info, subtitle, test)
