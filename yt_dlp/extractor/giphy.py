...
class GiphyBaseIE(InfoExtractor):
    ...
    _GIPHY_FE_STORIES_AND_TV_API_KEY = '3eFQvabDx69SMoOemSPiYfh9FY0nzO9x'
    _GIPHY_MAX = 5000

    def _fix_quote(self, string):
        ...
        for format_id in media:
            if ('_still' in format_id) == is_still:
                for key in [k for k, v in
                            media[format_id].items() if str(v)[:4] == 'http']:
    ...
    def _extract_info(self, gif_data, video_id, webpage=''):
        ...
        if data := gif_data.get('images'):
            if data.get('looping'):
                data['looping']['height'] = traverse_obj(
                    data, ('original_mp4', 'height', {int_or_none}))
                data['looping']['width'] = traverse_obj(
                    data, ('original_mp4', 'width', {int_or_none}))
        ...
        for _, v in subtitles.items():
            for sub in v:
                sub.setdefault('http_headers', {})['Accept'] = 'text/*'
        for t in thumbnails:
            t.setdefault('http_headers', {})['Accept'] = 'image/*'
        # tags
        if data := gif_data.get('tags'):
            if isinstance(data, str):
                idx = data.replace('$', '')
                if json_str := self._html_search_regex(
                        rf'"{idx}:(\[.*?\])', webpage, 'tags', default=None):
                    json_str = re.sub(
                        r'"\]\)self\.__next_f\.push\(\[\d+,"', '', json_str)
                    gif_data['tags'] = self._parse_json(
                        self._fix_quote(json_str), video_id, fatal=False)
        # uploader
            ...
            if isinstance(data, dict):
                uploader = traverse_obj(data, {
                    ...
                    'uploader_url': (('profile_url', 'website_url'),
                                     {lambda x: f'https://giphy.com{x}'
                                      if x and x[0] == '/' else url_or_none(x)}),
                }, get_all=False)
        # basic info
        info = {
            **traverse_obj(gif_data, {
                ...
                'description': ((None, 'video'), ('alt_text', 'description'),
                                {lambda x: x.strip()
                                 if x and not x.startswith('Discover & share') else None}),
                'tags': (('tags', 'user_tags', 'featured_tags'), {list}),
    ...
    def _api_channel_feed(self, channel_id):
        ...
        for _ in itertools.count(1):
            search_results = self._download_json(
                query_url, channel_id, fatal=False,
            ...
            if count >= (int_or_none(self.get_param('playlistend'))
                         or (self._GIPHY_MAX + 1)):
                return
            query_url = url_or_none(search_results.get('next')) or ''
            offset = int(self._search_regex(
                r'offset=(\d+)', query_url, 'offset', default=0))
...
class GiphyIE(GiphyBaseIE):
    ...
    _TESTS = [{
        'url': 'https://giphy.com/gifs/cats-flip-musical-l2JIcQ4UH5SoPtMJi',
        'info_dict': {
            ...
            'uploader_url': r're:https://giphy.com/(?:channel/)?catsmusical/',
        },
    }, {
        ...
        'info_dict': {
            ...
            'uploader': 'gus123',
            'uploader_id': 'gus123',
            'uploader_url': r're:https://giphy.com/(?:channel/)?gus123/',
        },
    }, {
        ...
        'info_dict': {
            ...
            'uploader_url': r're:https://giphy.com/(?:channel/)?digitalpratik/',
        },
    }, {
        ...
        'info_dict': {
            ...
            'uploader_url': r're:https://giphy.com/(?:channel/)?southpark/',
        },
    }, {
        ...
        'info_dict': {
            ...
            'uploader': 'NETFLIX',
            'uploader_id': 'netflix',
            'uploader_url': r're:https://giphy.com/(?:channel/)?netflix/',
        },
    }]
    ...
    def _real_extract(self, url):
        ...
        if title := (self._og_search_title(webpage, default=None)
                     or self._html_search_meta('twitter:title', webpage, default=None)):
            title = title.replace('- Find & Share on GIPHY', '').strip()
        description = (self._html_search_meta('twitter:description', webpage, default=None)
                       or self._og_search_description(webpage, default='Discover & share'))
        description = description if not description.startswith('Discover & share') else None

        gif_data = {}
        # search for:  \"gif\":{\"type\":\"...},
        if json_str := self._html_search_regex(
                r'\\"\w+\\":({\\"type\\":\\"(?!emoji).*?is_dynamic\\":\w+}),',
                webpage, 'video_data', default=None):
            gif_data = self._parse_json(self._fix_quote(json_str), video_id)
        # search for:  gif: {"...},
        elif json_str := self._html_search_regex(
                r'\s+\w+:\s*({".*?}),\n\s+', webpage, 'video_data', default='{}'):
            gif_data = self._parse_json(json_str, video_id)

        info = self._extract_info(gif_data, video_id, webpage=webpage)
        ...
        if not info.get('formats'):
            ...
            if url := self._og_search_video_url(webpage, default=None):
                formats.append({
                    ...
                    'width': int_or_none(
                        self._og_search_property('video:width', webpage)),
                    'height': int_or_none(
                        self._og_search_property('video:height', webpage)),
                    'url': url,
                })
            if url := self._og_search_thumbnail(webpage, default=None):
                formats.append({
                    'format_id': determine_ext(url),
                    'width': int_or_none(
                        self._og_search_property('image:width', webpage)),
                    'height': int_or_none(
                        self._og_search_property('image:height', webpage)),
                    'url': url,
                })
            if url := self._html_search_meta('twitter:image', webpage, default=None):
                thumbnails = [{
                    'width': int_or_none(
                        self._html_search_meta('twitter:image:width', webpage, default=None)),
                    'height': int_or_none(
                        self._html_search_meta('twitter:image:height', webpage, default=None)),
                    'url': url,
                }]
        ...
        if not info.get('uploader'):
            ...
            if not uploader:
                up_id = (gif_data.get('username')
                         or self._html_search_regex(r'<div>@(\w+)</div>',
                                                    webpage, 'uploader_id', default=None)
                         or self._html_search_regex(r'"woff2"/><link[^>]+\.giphy\.com/(?:channel_assets|avatars)/(.+?)/',
                                                    webpage, 'uploader_id', default=None))
                up_name = (title[(title.rfind(' by ') + 4):]
                           if title and title.rfind(' by ') > 0 else None
                           or self._html_search_regex(r'(?s)<h2\b[^>]*>([^<]+)</h2>',
                                                      webpage, 'uploader', default=None)
                           or self._html_search_regex(r'twitter:creator"[^>]+="((?!@giphy").*?)"',
                                                      webpage, 'uploader', default=None)
                           or up_id)
                uploader = {
                    'uploader': up_name,
            ...
            info = merge_dicts(
                info, {**{k: v for k, v in uploader.items() if v is not None}})
...
class GiphyChannelPageIE(GiphyBaseIE):
    ...
    _TESTS = [{
        'url': 'https://giphy.com/officialblueytv',
        'playlist_mincount': 10,
        'info_dict': {
            'id': '10295453',
            'title': 'Bluey',
            'uploader_id': 'giphy',
            'uploader_url': 'https://giphy.com/channel/officialblueytv',
        },
    }, {
        'url': 'https://giphy.com/channel/officialblueytv',
        'playlist_mincount': 10,
        'info_dict': {
            'id': '10295453',
            'title': 'Bluey',
    ...
    def _real_extract(self, url):
        ...
        # search for: "channelId":..., or "channel_id":..., or "channel":{"id":..., or \"channel\":{\"id\":...,
        if channel_info := self._search_regex(
                r'({\\?"channel[_\W]*[iI]d\\?":[^}]+})', webpage, 'channel_info',
                default=None):
            channel_info = channel_info + '}' * (
                channel_info.count('{') - channel_info.count('}'))
            channel_dict = self._parse_json(self._fix_quote(channel_info), video_id)
            info = traverse_obj(channel_dict['channel'], {
                'id': ('id', {int_or_none}, {lambda v: str(v) if v else None}),
                'uploader_id': ('display_name', {str_or_none}),
                'uploader_url': ('url', {lambda v: f'https://giphy.com/channel{v}'
                                         if v else None}),
            })
            if channel_id := info.get('id'):
                uploader_id = self._html_search_meta(
                    'twitter:creator', webpage, default='').replace('@', '').lower()
                entries = []
                ...
                return merge_dicts({
                    ...
                    'title': (self._og_search_title(webpage, default=None)
                              or self._html_search_meta('twitter:title', webpage,
                                                        default='GIFs on GIPHY - Be Animated')
                              ).replace('GIFs on GIPHY - Be Animated', '').strip(),
                    'uploader_id': uploader_id,
                    'uploader_url': (f'https://giphy.com/channel/{uploader_id}'
                                     if uploader_id != 'giphy' else None),
                    '_type': 'playlist',
                    'entries': entries,
                }, info)
...
class GiphyChannelIE(GiphyBaseIE, SearchInfoExtractor):
    ...
    def _search_results(self, query):
        if webpage := self._download_webpage(
                f'https://giphy.com/channel/{query}', query):
            if channel_id := self._html_search_regex(
                    r'"channel[_\W]*[iI]d\\?":\s*"?([^",\}]+)[",\}]', webpage,
    ...
    def _search_results(self, query):
        def search_query(query, offset, limit, category):
            ...
            return self._download_json(
                ...
                note=f'Downloading {category} result {offset + 1}-{offset + limit}',
                query={
        ...
        types = self._search_regex(
            r'&type=([^&]+)', query, 'type', default='gifs,stickers,videos')
        types = [(f'{x}s' if x[-1] != 's' and any(
                  x in t for t in ['clips', 'gifs', 'stickers', 'videos']) else x)
...
class GiphyStoriesIE(GiphyBaseIE):
    ...
    def _real_extract(self, url):
        ...
        story = self._download_json(
            f'https://x.giphy.com/v1/stories/slug/{slug}?api_key={self._GIPHY_FE_STORIES_AND_TV_API_KEY}', slug)
        ...
        if data := story.get('data'):
            ...
            info = traverse_obj(data, {
                ...
                'tags': ('tags', {list}),
                'thumbnails': ('cover_gif', 'gif', 'images', {dict},
                               {lambda x: self._extract_formats(x, is_still=True)}),
