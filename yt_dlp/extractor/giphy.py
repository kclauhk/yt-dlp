        ...
        if data := gif_data.get('tags'):
            if isinstance(data, str):
                ...
                if json_str := self._html_search_regex(rf'"{idx}:(\[.*?\])',
                    ...
                    gif_data['tags'] = self._parse_json(json_str, video_id, fatal=False)
        # uploader
...
class GiphyIE(GiphyBaseIE):
    ...
    def _real_extract(self, url):
        ...
        title = title.replace('- Find & Share on GIPHY', '').strip() if title else None
                ...
                if isinstance(data, str):
                    ...
                    if isinstance(data, dict):
                        uploader = traverse_obj(data, {
                            ...
                            'uploader_url': (('profile_url', 'website_url'),
                                             {lambda x: f'https://giphy.com{x}' if x and x[0] == '/'
                                              else url_or_none(x)}),
                        }, get_all=False)
            if not uploader:
                up_id = (gif_data.get('username')
                         or self._html_search_regex(r'<div>@(\w+)</div>', webpage,
                ...
                up_name = (title[(title.rfind(' by ') + 4):] if title and title.rfind(' by ') > 0 else None
...
class GiphyChannelPageIE(GiphyBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if channel_info := self._search_regex(r'({\\?"channel[_\W]*[iI]d\\?":[^}]+})',
                                              webpage, 'channel_info', default=None):
            channel_info = channel_info + '}' * (channel_info.count('{') - channel_info.count('}'))
            channel_dict = self._parse_json(channel_info.replace(r'\"', '"'), video_id)
            info = traverse_obj(channel_dict['channel'], {
                ...
                'uploader_url': ('url', {lambda v: f'https://giphy.com/channel{v}' if v else None}),
            })
...
class GiphyChannelIE(GiphyBaseIE, SearchInfoExtractor):
    ...
    def _search_results(self, query):
        if webpage := self._download_webpage(f'https://giphy.com/channel/{query}', query):
            if channel_id := self._html_search_regex(r'"channel[_\W]*[iI]d\\?":\s*"?([^",\}]+)[",\}]',
