...
class MediasiteIE(InfoExtractor):
        ...
        for i, slide in enumerate(stream['Slides']):
            ...
            fragments.append({
                'path': join_nonempty(fname_template.format(slide.get('Number', i + 1)),
                                      stream.get('SlidePlaybackTicketId'), delim='?playbackTicket='),
    ...
    def _real_extract(self, url):
        ...
        for i, x in enumerate(['PodcastUrl', 'VodcastUrl']):
            if url_or_none(presentation.get(x)):
                formats.append({
                    'format_id': x.lower().replace('url', ''),
                    'url': presentation.get(x).split('?attachmentName=')[0],
