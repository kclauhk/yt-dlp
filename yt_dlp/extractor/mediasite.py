...
class MediasiteChannelIE(InfoExtractor):
    ...
    {
        'url': 'https://ers.mediasite.com/mediasite/Channel/august_ers_meeting/browse/audit/relevance/null/0/null',
        'info_dict': {
    ...
    def _entries(self, json_data, channel_id, query):
        ...
        for i in itertools.count():
            postdata = {
                ...
                'SearchBy': query[0],
                'SearchTerm': query[0],
                'Tags': [query[2]] if query[2] else None,
    ...
    def _real_extract(self, url):
        ...
        if query := mobj.group('query'):
            query = [None if x.lower() == 'null' else urllib.parse.unquote(x) for x in query.split('/')]
