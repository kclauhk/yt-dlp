...
class BlueyIE(InfoExtractor):
    ...
    _TESTS = [{
        # Episodes page (youtube embeded: https://youtu.be/u6D2ucvSas0)
        ...
        # Episodes page with trailer video
        ...
        # Book-reads page (youtube embeded: https://youtu.be/NbLxoLyPGyc)
        ...
        # Bonus-bits page (youtube embeded: https://youtu.be/UUkb_b5UEE0)
    ...
    def _real_extract(self, url):
        ...
        def brightcove_api(brightcove_id, video_id):
            ...
            if data := self._download_json(f'https://edge.api.brightcove.com/playback/v1/accounts/6041795457001/videos/{brightcove_id}',
                ...
                return {
                    **traverse_obj(data, {
                        'id': ('id', {str_or_none}, {lambda x: x or video_id}),
                        ...
                        'tags': ('tags', {list}),
                        'upload_date': (('published_at', 'created_at'), {str_or_none},
            ...
            info = {
                ...
                **traverse_obj(json_ld[-1], {
                    'title': (('containsSeason', '@graph'), 0, (('episode', 'name'), 'name'),
                              {str_or_none}, {lambda x: x.split(' | ')[-1]}),
                    ...
                    'season': ('containsSeason', 0, 'name', {str_or_none}, {lambda x: x if re.match(r'Season ', x) else None}),
                    'season_number': ('containsSeason', 0, 'name', {str_or_none},
                                      {lambda x: int(x.replace('Season ', '')) if re.match(r'Season \d+$', x) else None}),
                    'episode': ('containsSeason', 0, 'episode', 'episodeNumber', {str_or_none},
                                {lambda x: f'Episode {x}' if x else None}),
                    'episode_number': ('containsSeason', 0, 'episode', 'episodeNumber', {int_or_none}),
                }, get_all=False),
                'upload_date': traverse_obj(json_ld[0], ('@graph', 0, 'datePublished', {str_or_none},
