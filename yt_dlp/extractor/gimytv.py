...
class GimyIE(InfoExtractor):
    IE_NAME = 'gimy:ep'
    _VALID_URL = r'(gimy:|https?://gimytv\.ai/ep/)(?P<id>\d+-\d+-\d+)(?:/|\.html)?$'
    _TESTS = [{
        ...
        for playlist in [x for x in playlists if x['source'] != given_source]:
            for item in playlist['links']:
                ...
                for e in episode:
                    ...
                    if item not in result and e in ep_item:
                        z.append(item)
                    if (e[2] == 'd'
                    ...
                    if len(z) == 1:
                        ...
                        result.append(d[0])
                    if (len(z) == 1 or len(d) == 1) and len(ep_item) > 1:
                        for e in ep_item:
                            if e not in episode:
                                episode.append(e)
        return result
    ...
    def _search_results(self, query):
        ...
        for page_number in itertools.count(1):
            webpage = self._download_webpage(
                ...
                note=f'Downloading result page {page_number}', tries=2).replace('&nbsp;', ' ')
            if '<title>404 not found</title>' in webpage:
                raise ExtractorError(
                    'Unable to download webpage: HTTP Error 404: Not Found (caused by <HTTPError 404: Not Found>)', expected=True)
            ...
            for result in search_results:
                yield self.url_result(self._BASE_URL + result, GimyVdoIE)
