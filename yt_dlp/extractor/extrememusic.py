...
class ExtremeMusicBaseIE(InfoExtractor):
    ...
    def _get_album(self, album_id, video_id, fatal=True):
        album = self._download_json(f'{self._API_URL}/albums/{album_id}', video_id,
                                    fatal=fatal, note='Downloading album data',
                                    errnote='Unable to download album data',
                                    headers=self._REQUEST_HEADERS) or {}
        for k, url in traverse_obj(album, ('album', 'video')).items():
            if url_or_none(url) and determine_ext(url) == 'm3u8':
                for f in self._extract_m3u8_formats(
                        url, video_id, 'mp4', fatal=False, note='Downloading album data',
                        m3u8_id=f'artwork_{k}', errnote='Unable to download album data'):
                    album['album'].setdefault('artwork', []).append({
                        **f,
                        'vcodec': 'none',
                        'acodec': 'none',
                    })
        if video_id == album_id:
            bio = self._download_json(f'{self._API_URL}/albums/{album_id}/bio', video_id,
                                      fatal=False, note='Downloading album data',
                                      errnote='Unable to download album data',
                                      headers=self._REQUEST_HEADERS) or {}
        ...
        if 'tracks' in album_data and 'track_sounds' in album_data:
            if not track_id and version_id:
                track_id = traverse_obj(album_data, (
                    'track_sounds', lambda _, v: v['id'] == int(version_id),
                    'track_id', {int}), get_all=False)
            if track := traverse_obj(album_data, (
                ...
                track_info = {**traverse_obj(track, {
                    ...
                    'description': (
                        'description',
                        {lambda v: v.removeprefix('Full version - ') if str_or_none(v) else None}),
                    'artists': (
                        'artists',
                        {lambda v: v or traverse_obj(album_data, ('album', 'artist'))},
                ...
                if not self._REQUIRE_VERSION:
                    version_id = (version_id
                                  or traverse_obj(track, 'default_track_sound_id',
                                                  ('track_sound_ids', 0)))
                for sound_id in [version_id] if version_id else track['track_sound_ids']:
                    if sound := traverse_obj(album_data, (
                            'track_sounds', (lambda _, v: v['id'] == int(sound_id)
                                             and v['track_id'] == int(track_id))),
                                             get_all=False):
                        if (version_id
                                or 'all' in self._REQUIRE_VERSION
                                or any(x in sound['version_type'].lower()
                                       for x in self._REQUIRE_VERSION)):
                            formats = traverse_obj(
                                album_data, ('album', 'artwork'), default=[])[:]
                            for audio_url in traverse_obj(sound, (
                                    'assets', 'audio', ('preview_url', 'preview_url_hls'))):
                                if determine_ext(audio_url) == 'm3u8':
                                    m3u8_url = re.sub(r'\.m3u8\?.*', '/HLS/128_v4.m3u8', audio_url)
                                    for f in self._extract_m3u8_formats(
                                            m3u8_url, sound_id, 'm4a', m3u8_id='hls', fatal=False):
                                        formats.append({
                                            **f,
                                            'vcodec': 'none',
                                        })
                                    ...
                                    formats.append({
                                        'format_id': determine_ext(audio_url).lower(),
                                        'url': audio_url,
                                        'vcodec': 'none',
                                    })
                            entries.append({
                                'id': str(sound_id),
                                'title': join_nonempty('title', 'version_type',
                                                       from_dict=sound, delim=' - '),
...
class ExtremeMusicIE(ExtremeMusicBaseIE):
    ...
    _TESTS = [{
        ...
        'info_dict': {
            ...
            'track_number': 5,
            'track_id': 'HPE316_05',
            'description': r're:a emotive, chill and reflective pop track',
            'artists': ['PRAERS'],
        ...
        'info_dict': {
            ...
            'track_number': 2,
            'track_id': 'ASM0002_02',
            'description': r're:a fun, happy and upbeat pop track',
            'composers': ['Keita Sone'],
        ...
        'info_dict': {
            ...
            'track_number': 2,
            'track_id': 'HPE263_02',
            'description': r're:a fun, sexy and confident pop track',
        ...
        'url': 'https://www.extrememusic.com/albums/15064?item=254704&ver=1178852',
        'info_dict': {
            'id': '1178852',
            'ext': 'mp3',
            'title': 'SWEET TOOTH - Instrumental',
            'alt_title': 'Instrumental',
            'track': 'SWEET TOOTH',
            'track_number': 2,
            'track_id': 'HPE263_02',
    ...
    def _real_extract(self, url):
        ...
        album_data = self._get_album(album_id, version_id or track_id)
...
class ExtremeMusicAIE(ExtremeMusicBaseIE):
    ...
    _TESTS = [{
        'url': 'https://www.extrememusic.com/albums/3821',
        'info_dict': {
            'id': '3821',
            'album': 'MOVING ON',
            'description': r're:Known for infusing J-pop or “Kawaii” inspired productions with diverse EDM genres,',
            'artists': ['MYLK'],
            'genres': ['POP', 'JPOP'],
    ...
    def _real_extract(self, url):
        ...
        album_info = merge_dicts(traverse_obj(album_data, ('album', {
            ...
            'tags': ('keywords', {lambda v: v.split(', ') if v else None}),
        })), {
            'description': traverse_obj(
                album_data, ('bio', 'description', {str_or_none})),
        })

        return self.playlist_result(
            (entry for sounds in (
                track[0] for track in (
                    self._extract_track(album_data, track_id=track_id)
                    for track_id in traverse_obj(
                        album_data, ('tracks', ..., 'id'))))
             for entry in sounds),
            **album_info)
            ...
            for i in itertools.count():
                try:
                    ...
                    playlist = playlist_query(
                        playlist_id.split('_')[0], i * limit, limit,
                        note=f'Downloading item {i * limit + 1}-{i * limit + limit}')
                except Exception:
                    playlist = playlist_query(
                        playlist_id, i * limit, limit,
                        note=f'Downloading item {i * limit + 1}-{i * limit + limit}')
                if playlist_items_count := traverse_obj(
                        playlist, ('playlist', 'playlist_items_count')):
                    if len(playlist.get('tracks', [])) == 0:
                        if items_count < playlist_items_count:
                            self.report_warning(
                                'This playlist has geo-restricted items. '
                                'Try using --xff to specify a different country code')
                        break
                    else:
                        ...
                        for track_id in list(dict.fromkeys(track_ids)):
                            if track_id not in tracks_done:
                                ...
                                if album_id not in albums:
                                    albums[album_id] = self._get_album(album_id, track_id, fatal=False)
                                playlist['album'] = traverse_obj(albums, (album_id, 'album'))
...
class ExtremeMusicSearchIE(ExtremeMusicBaseIE, SearchInfoExtractor):
    IE_NAME = 'ExtremeMusic:search'
    _SEARCH_KEY = 'exmsearch'
    _TESTS = [{
        'url': 'exmsearch5:latin pop',
        'info_dict': {
            'id': 'latin pop',
            'title': 'latin pop',
        },
        ...
        'url': 'exmsearchall:pop romantic emotive',
        'info_dict': {
            'id': 'pop romantic emotive',
            'title': 'pop romantic emotive',
        },
    ...
    def _search_results(self, query):
        # search api:
        # https://snapi.extrememusic.com/search/tracks?facets=1&field=&isAiSearch=false&mode=filter&query={query}&range=40%2C20
        query_url = f'{self._API_URL}/search/tracks?facets=1&field=&isAiSearch=false&mode=filter&query={urllib.parse.quote(query)}'
        country = (self.get_param('geo_bypass_country')
                   or self._download_webpage('http://ip-api.com/csv/?fields=countryCode',
                                             query, note='Detecting your location')
                   or 'DE')
        self._initialize(query_url, query, country)

        def search_query(query, page, items_per_page):
            r = urllib.parse.quote(f'{(page - 1) * items_per_page},{items_per_page}')
            return self._download_json(
                f'{query_url}&range={r}', query, headers=self._REQUEST_HEADERS,
                note=f'Downloading result {(page - 1) * items_per_page + 1}-{page * items_per_page}')

        items_per_page = 20
        for i in itertools.count(start=1, step=1):
            search_results = search_query(query, i, items_per_page)
            total_count = traverse_obj(
                search_results, ('meta', 'total_count', {int}), default=0)
            for track in traverse_obj(search_results, ('track_search_result_items')):
                yield self.url_result(
                    f"https://www.extrememusic.com/albums/{track['album_id']}?item={track['track_id']}",
                    **traverse_obj(track, {
                        'title': ('title', {lambda v: v.title()}),
                        'track_id': ('track_cat_no', {lambda v: v.upper()}),
                        'album': ('album_title', {lambda v: v.title()}),
                        'thumbnail': ('images', 'default', 0, 'url', {url_or_none}),
