...
class ExtremeMusicBaseIE(InfoExtractor):
    ...
    def _initialize(self, url, video_id, country=None):
        ...
        env = self._download_json('https://www.extrememusic.com/env', None, note='Get API token')
        ...
        if 'tracks' in album_data and 'track_sounds' in album_data:
            ...
            if track := traverse_obj(album_data, (
                ...
                track_info = {**traverse_obj(track, {
                    ...
                    'description': ('description', {lambda v: v.removeprefix('Full version - ') if str_or_none(v) else None}),
...
class ExtremeMusicSearchIE(ExtremeMusicBaseIE, SearchInfoExtractor):
    ...
    _SEARCH_KEY = 'extrememusicsearch'
    _TESTS = [{
        'url': 'extrememusicsearch5:latin pop moody',
        'info_dict': {
            'id': 'latin pop moody',
            'title': 'latin pop moody',
        },
        'playlist_count': 5,
    }, {
        'url': 'extrememusicsearchall:latin pop',
        'info_dict': {
            'id': 'latin pop',
            'title': 'latin pop',
        },
    ...
    def _search_results(self, query):
        ...
        country = (self.get_param('geo_bypass_country')
                   or self._download_webpage('http://ip-api.com/csv/?fields=countryCode',
                                             None, note='Get country code')
                   or 'DE')
        self._initialize(query_url, query, country)

        def search_query(query, page, items_per_page):
            r = urllib.parse.quote(f'{page * items_per_page},{items_per_page}')
            return self._download_json(
                f'{query_url}&range={r}', query, headers=self._REQUEST_HEADERS,
                note=f'Downloading result {page * items_per_page + 1}-{(page + 1) * items_per_page}')
