    ...
    def extract_formats(self, play_info):
        ...
        for audio in audios:
            for url in sorted([u for u in dict.fromkeys(traverse_obj(
                    audio, (('baseUrl', 'base_url', (('backupUrl', 'backup_url'), ...), 'url'),
                            {url_or_none}))) if u], reverse=True):
                formats.append({
                    'url': url,
                    ...
                    'format_note': '({})'.format(self._search_regex(r'://([^\.]+)', url, None, fatal=False)[:7]),
                })
        for video in traverse_obj(play_info, ('dash', 'video', ...)):
            for url in sorted([u for u in dict.fromkeys(traverse_obj(
                    video, (('baseUrl', 'base_url', (('backupUrl', 'backup_url'), ...), 'url'),
                            {url_or_none}))) if u], reverse=True):
                formats.append({
                    'url': url,
    ...
    def _real_extract(self, url):
        ...
        if self.is_logged_in:
            play_info = traverse_obj(
                ...
                ('data', {dict}))
        if not play_info:
            play_info = self._download_playinfo(video_id, cid, headers=headers, query={'try_look': 1, 'avid': aid})
        formats = self.extract_formats(play_info)
        ...
        if not traverse_obj(play_info, 'dash'):
            ...
            for qn in traverse_obj(play_info, ('accept_quality', lambda _, v: not has_qn(v), {int})):
                formats.extend(traverse_obj(
                    self.extract_formats(self._download_playinfo(video_id, cid, headers=headers, query={'qn': qn, 'avid': aid})),
