...
class GimyIE(InfoExtractor):
    ...
    def _check_404_error(self, webpage):
    ...
    if nodriver_available:
        ...
        def _download_webpage(
            ...
            async def __download_webpage(url):
                        ...
                        while (loop_count < 10):
                            if error := self._search_regex(
                                ...
                                raise TransportError(
                                    f'Unable to download webpage: HTTP Error:  (caused by <{error}>)')
                                break
                            elif ('</html>' not in content
                                  or ('<title>404 not found</title>' not in content
                                      and ' class="video-' not in content)):
            ...
            return self._loop.run_until_complete(__download_webpage(url))
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(url, video_id, note=False, tries=3)
        self._check_404_error(webpage)
        ...
        for src in self._extract_other_src(self._extract_playlists(webpage),
            ...
            if src['url'] != url:
                page = self._download_webpage(src['url'], video_id, note=False, tries=3)
                if player_data := self._search_json(
...
class GimyVodIE(GimyIE):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            f'{self._BASE_URL}/vod/{video_id}.html', video_id, note=False, tries=3)
        self._browser.stop()
        self._check_404_error(webpage)
...
class GimySearchIE(SearchInfoExtractor, GimyIE):
    ...
    def _search_results(self, query):
        ...
        for page_number in itertools.count(1):
            webpage = self._download_webpage(
                f'{self._BASE_URL}/search/{keywords}----------{page_number}---.html', query,
