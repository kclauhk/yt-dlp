...
class GimyIE(InfoExtractor):
    ...
    _TESTS = [{
        'url': 'https://gimy.cc/video/26198-2-128.html',
        'info_dict': {
            'id': '26198-2-128',
            'title': 'Running Man - 20240407',
            'description': 'md5:d5b252c4c6430d22c567aa8cb8bf0052',
            'episode': '20240407',
            'thumbnail': r're:^https?://.*',
            'release_year': 2010,
            'upload_date': '20240413',
            'catagory': ['綜藝'],
            'region': '韓國',
            'ext': 'mp4',
            'duration': 5466.28 if FFmpegPostProcessor().probe_available else None,
        },
    }]
            ...
            if f:
                ...
                if f[0]['url'].count('.haiwaikan.com') > 0:
                    ...
                    if ffmpeg.probe_available:
                        data = ffmpeg.get_metadata_object(f[0]['url'])
                        if data:
                            if data['format']:
                                f[0].update({
                                    'filesize': int_or_none(data['format'].get('size')),
                                    'duration': round(float(data['format']['duration']), 2) if data['format'].get('duration') else None,
                                    'tbr': float_or_none(data['format'].get('bit_rate'), 1000),
                                })
                            for stream in data['streams']:
                                if stream['codec_type'] == 'video':
                                    ...
                                    f[0].update({
                                        ...
                                        'vbr': float_or_none(stream.get('bit_rate'), 1000),
                                        'video_ext': 'mp4',
                                    })
                                elif stream['codec_type'] == 'audio':
                                    f[0].update({
                                        'audio_channels': int_or_none(stream.get('channels')),
                                        'acodec': stream.get('codec_name'),
                                        'asr': int_or_none(stream.get('sample_rate')),
                                        'abr': float_or_none(stream.get('bit_rate'), 1000),
                                    })
                return {'formats': [f[0]]}
            else:
                return {}
...
class GimyPlaylistIE(GimyIE):
    IE_NAME = GimyIE.IE_NAME + ':playlist'
    _VALID_URL = r'(gimy:|https?://gimy\.cc/detail/)(?P<id>\d+)'
    _TESTS = [{
        'url': 'https://gimy.cc/detail/241529.html',
        'info_dict': {
            'id': '241529',
            'title': '魔法科高校的劣等生第三季',
            'description': 'md5:940f6decd4e62e01388a40620e56cd75',
            'thumbnail': r're:^https?://.*',
            'release_year': 2024,
            'catagory': ['動漫'],
            'region': '日本',
...
class GimySearchIE(GimyIE):
    ...
    _TESTS = [{
        'url': 'https://gimy.cc/search.html?wd=2024',
        'info_dict': {
            'id': '2024',
            'title': '2024搜索結果',
            'description': '2024搜索結果',
        },
        'playlist_count': 123,
    }, {
    ...
    def _real_extract(self, url):
        ...
        if 'aks-404-page' not in webpage:
            ...
            entries = [self.url_result('https://gimy.cc' + x, GimyPlaylistIE)
