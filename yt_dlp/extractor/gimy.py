...
class GimyIE(InfoExtractor):
    ...
    def _extract_formats(self, player_data, video_id, source_name, episode):
        ...
        if media_url := traverse_obj(player_data, ('url', {str_or_none})):
            if media_url[:3] == 'JD-':  # or media_url[-5:] == '.html':    m3u8 from '.html' responses 404 error
                parse_type = 'dp' if media_url[:3] == 'JD-' else 'u'
                jd_data = (self._download_json(
                    f'https://player.gimy.bot/{parse_type}/parse.php?url={media_url}&_t={int(time.time())}',
                    video_id, note='Downloading player data')
                    or self._download_json(
                    f'https://play.gimymax.com/{parse_type}/parse.php?url={media_url}&_t={int(time.time())}',
                    video_id, note='Downloading player data'))
                media_url = traverse_obj(jd_data, ('url', {url_or_none}))
            if url_or_none(media_url):
                ...
                if all(x not in media_url for x in skipped_sources):
                    for f in self._extract_m3u8_formats_and_subtitles(
                        ...
                        f.setdefault('http_headers', {})['Origin'] = 'https://player.gimy.bot'
                        yield f
