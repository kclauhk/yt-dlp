    ...
    if nd_available:
        ...
        def _is_loaded(html):
            return (('劇迷' in html or '<title>404 not found</title>' in html)
                    and '首頁' in html and '</html>' in html)
                ...
                while True:
                    try:
                        ...
                        while loop_count < (1 + (not try_count)) * 5:
                            if is_loaded(content):
                                break
                            elif error := self._search_regex(
            ...
            if response.get_header('server').lower() == 'cloudflare':
                if response.get_header('cf-mitigated') == 'challenge':
                    ...
                    return self._download_webpage(
                        ('https://gimyai.tw/' + path[urllib.parse.urlparse(url).path[1]]
                         + f'/{url_basename(url)}'), video_id, note, errnote, fatal,
