...
class GimyIE(InfoExtractor):
    ...
    def _extract_formats(self, media_url, video_id, episode, source):
        ...
        if url_or_none(media_url):
            skipped_sources = ['.html', '.ppqrrs.com', '.yaaabc.com', '.wsyzym3u8.com', '.qsstvw.com',
                               '.hhiklm.com', '.zuidazym3u8.com', '.daayee.com', '.bxgbnet.com', 'v6.qrssuv.com']
    ...
    if nodriver_available:
        ...
        def _nodriver_download_webpage(
                self, url, video_id, note=None, errnote=None, fatal=True,
                tries=3, timeout=0, matching_func=_content_exist, *args, **kwargs):
    ...
    def _download_webpage_wrapper(
        ...
        try:
            ...
            if (res.get_header('cf-mitigated') == 'challenge'
                ...
                if nodriver_available:
                    self._download_webpage_wrapper = self._nodriver_download_webpage
                    return self._download_webpage_wrapper(
                        url, video_id, note, errnote, fatal, 3, timeout, *args, **kwargs)
                else:
                    raise TransportError('Got HTTP Error 403 caused by Cloudflare anti-bot challenge')
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage_wrapper(url, video_id)
        ...
        for src in self._extract_other_src(self._extract_playlists(webpage),
            ...
            if src['url'] != url:
                page = self._download_webpage_wrapper(src['url'], video_id, f'Extracting URL: {src["url"]}')
                if player_data := self._search_json(
...
class GimySearchIE(SearchInfoExtractor, GimyIE):
    ...
    def _search_results(self, query):
        ...
        for page_number in itertools.count(1):
            webpage = self._download_webpage_wrapper(
