...
class GimyIE(InfoExtractor):
    ...
    def _extract_formats(self, player_data, video_id, source_name, episode):
        ...
        def parse_url(media_url):
            ...
            if media_url[:3] in types:
                parse_type = types[media_url[:3]]
            else:
                parse_type = types['url']
            # https://player.gimy.bot/u/parse.php?url={media_url}&jctype=etcjc&next=//gimyplus.com/ep/454135-3-2.html
            # https://player.gimy.bot/d/parse.php?url={media_url}&_t=1781844164145
            # https://player.gimy.bot/i/parse.php?verify=1&url={media_url}&_t=1778152814109
            # https://player.gimy.bot/n/parse.php?url={media_url}&jctype=NSYS&next=gimyplus.com/ep/256836-12-4.html
            # https://v.gimy.bot/j[xd]/api.php?url={media_url}
            return [
                f'https://player.gimy.bot/{parse_type[0]}/{parse_type[2]}?url={media_url}{parse_type[3]}',
                f'https://play.gimy01.tv/{parse_type[1]}/{parse_type[2]}?url={media_url}{parse_type[3]}',
                f'https://v.gimy.bot/{parse_type[4]}/api.php?url={media_url}',
            ]
        ...
        if vdo_url := traverse_obj(player_data, ('url', {str_or_none})):
            ...
            parser_url = None
            if not url_or_none(vdo_url) or vdo_url[-5:] != '.m3u8':
                for url in parse_url(vdo_url):
                    if player_data := self._download_json(
                            f'{url}&_t={int(time.time() * 1000)}',
                            video_id, note='Downloading player data', fatal=False):
            ...
            if url_or_none(vdo_url):
                skipped_sources = ['.html', 'vodcnd04.oag7h.com', '.ppqrrs.com', '.ryplay4.com',
                                   '.yaaabc.com', '.zuidazym3u8.com', '.bxgbnet.com',
    ...
    if nd_available:
        ...
        def _is_loaded(html):
            return (('劇迷' in html or '<title>404 not found</title>' in html)
                    and '</html>' in html)
                ...
                while True:
                    try:
                        ...
                        while loop_count < (1 + (not try_count)) * 5:
                            ...
                            elif not is_loaded(content):
                                self.to_screen('Sleeping 3 seconds ...')
                                await self._browser.wait(3)
                                content = await webpage.get_content()
                                if "cType: 'interactive'" in content:
                                    await webpage.verify_cf()
                                loop_count += 1
                            else:
                                break
    ...
    def _download_webpage(
        ...
        try:
            ...
            if response.get_header('server').lower() == 'cloudflare':
                if response.get_header('cf-mitigated') == 'challenge':
                    if nd_available:
                        ...
                        raise ExtractorError(
                            ...
                            'try again after install Chrome, opencv-python and nodriver package '
                            '(https://github.com/UltrafunkAmsterdam/nodriver); '
                            'to install nodriver, use "pip install nodriver==0.47.0"',
                            expected=True)
                elif b'"unable_to_access"' in response.read():
                    self._BASE_URL = 'https://gimyai.tw'
                    return self._download_webpage(
                        'https://gimyai.tw/play/' + url_basename(url), video_id,
                        note, errnote, fatal, tries, timeout, *args, *kwargs)
            raise
        ...
        if '/parse.php?' in info.get('url') or '/api.php?' in info.get('url'):
            try:
                player_data = self._download_json(
                    ...
                    video_id, note='Obtaining m3u8 manifest URL')
                if player_data.get('code') == 200:
                ...
                if test:
                    ...
                    raise ExtractorError(f'[{self.IE_NAME}] {msg}')
        return info
