...
class GimyIE(InfoExtractor):
    IE_NAME = 'gimy:ep'
    _VALID_URL = r'(gimy:|(?P<base_url>https?://gimy[^/]*\.\w\w)/e[^/]*/)(?P<id>\d+-\d+-\d+)(?:/|\.html)?$'
    _TESTS = [{
    ...
    def _real_extract(self, url):
        ...
        cast, formats = [], []
        if player_data := self._extract_player_data(webpage, video_id):
            # cast
        ...
        for src in self._extract_other_src(self._extract_playlists(webpage),
            ...
            if src['url'] != url:
                ...
                if player_data := self._extract_player_data(page, video_id):
    ...
    def _before_download(self, info_dict, test):
        video_id = info_dict.get('id')
        if determine_ext(info_dict.get('url')) != 'm3u8':
            try:
                player_data = self._download_json(
...
class GimyVodIE(GimyIE):
    IE_NAME = 'gimy:vod'
    _VALID_URL = r'(gimy:|(?P<base_url>https?://gimy[^/]*\.\w\w)/v[^/]*/)(?P<id>\d+)(?:/|\.html)?$'
    _TESTS = [{
