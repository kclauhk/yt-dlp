...
class GimyIE(InfoExtractor):
    ...
    def _extract_media_url(self, player_data, video_id):
        url = traverse_obj(player_data, ('url', {str_or_none}))
        if url[:3] == 'JD-':
            jd_data = self._download_json(
                f'https://play.gimymax.com/dp/parse.php?url={url}&_t={int(time.time())}', video_id)
            url = traverse_obj(jd_data, ('url', {url_or_none}))
        return url_or_none(url), player_data.get('from')

    def _extract_formats(self, media_url, video_id, format_id, source, episode):
        """
        :return: generator -- formats
        """
        if url_or_none(media_url):
            skipped_sources = ['.html', '.hhiklm.com', 'vv.jisuzyv.com', 'vodcnd13.oag7h.com',
                               '.ppqrrs.com', '.zuidazym3u8.com', '.bxgbnet.com', '.daayee.com',
                               'v6.qrssuv.com', '.qsstvw.com', '.wsyzym3u8.com', '.yaaabc.com']
            if all(x not in media_url for x in skipped_sources):
                for f in self._extract_m3u8_formats_and_subtitles(
                    ...
                    f['format_id'] = format_id.lower() or self._html_search_regex(
                        r'https?://[^/]*?\.?([\w]{4,}|[^\.]+)[^\.]*\.\w+/',
    ...
    def _real_extract(self, url):
        ...
        current_src = self._html_search_regex(
            r'現在播放:\s*(.+?)\s*(?:- .+)?</span', webpage, 'video_src', default='0')
        vod_url, vod_from = self._extract_media_url(player_data, video_id)
        if f := list(self._extract_formats(vod_url, video_id, vod_from, current_src, episode)):
            formats = f
        # other video sources
        for src in self._extract_other_src(self._extract_playlists(webpage),
            ...
            if src['url'] != url:
                ...
                if player_data := self._search_json(
                    ...
                    vod_url, vod_from = self._extract_media_url(player_data, video_id)
                    if f := list(self._extract_formats(vod_url, video_id, vod_from,
                                                       src['source'], src['ep'])):
                        formats += f
