        ...
        for e in episode.split(' '):
            ...
            if re.search(r'(第\d+集)|(ep?\s*\d+)|(episode\s*\d+)|（\d+）', e, re.IGNORECASE):
                x.append((float(re.findall(r'(?:第|ep?\s*|episode\s*|（)+0?(\d+)[集）]?', e,
                                           re.IGNORECASE)[0]),
                          part, 'e'))
            elif re.search(r'^\D*\d{1,4}(?:[^K]\D)*$', e):
            ...
            if re.search(r'^(SD|HD|FHD|\D*\dK|\d{3,4}P|標清|超清|高清|正片|中字|TC)', e, re.IGNORECASE):
                x.append(('RES', part, 'r'))
            if len(x) == 0:
                x.append((e, part, 'n'))
            result = result + x
    ...
    def _extract_formats(self, player_data, video_id, source_name, episode):
        ...
        def get_parser_url(url):
            ...
            return [
                ...
                f'https://play.aigm.tv/{parse_type[4]}/api.php?url={url}',
        ...
        if video_url := traverse_obj(player_data, ('url', {str_or_none})):
            ...
            if url_or_none(video_url):
                skipped_sources = ['.html', 'vodcnd04.oag7h.com', '.ryplay4.com', 'svip.ryplay14.com',
...
class GimyVodIE(GimyIE):
    ...
    _TESTS = [{
        ...
        'info_dict': {
            'id': '414742',
            'title': '九龍城寨之梟雄崛起',
            'description': r're:重生前，霍凌被後媽楊欣與弟弟霍翔陷害，',
            'thumbnail': r're:https?://',
            'categories': ['短劇'],
            'cast': 'count:3',
            'view_count': int,
