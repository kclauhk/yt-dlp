...
class GimyIE(InfoExtractor):
    IE_NAME = 'gimy'
    _VALID_URL = r'(gimy:|https?://gimy\.(?:cc|la)/(?:index\.php/)?(?:play|video)/)(?P<id>\d+)[-/](?:(?P<source_id>\d+)-(?P<episode_id>\d+)|ep(?P<episode_la>\d+)\?sid=(?P<source_la>\d+))'
    _TESTS = [{
    ...
    def _parse_episode(self, string):
        ...
        for x in episode.split(' '):
            z = []
            if re.search(r'(19|20)?\d*\d{6}', x):
                z.append((re.findall(r'(?:19|20)?\d*(\d{6})', x)[0][-6:], part, 'd'))
    ...
    def _extract_formats(self, video_id, episode_label, media_source, video_url):
        ...
        if url_or_none(video_url):
            _skip_sources = ['.fsvod1.com', '.bfvvs.com', '.youkuplaya.com', 'v6.pptvlist.com']
    ...
    def _real_extract(self, url):
        ...
        vid, src_id, ep_id, src_la, ep_la = self._match_valid_url(url).group(
            'id', 'source_id', 'episode_id', 'source_la', 'episode_la')
        sid, eid = (src_id or src_la, ep_id or ep_la)
        video_id = str(vid) + '-' + str(sid) + '-' + str(eid)
        ...
        description = ((desc_meta[:desc_meta.index(intro[0:intro.find('。')])] + intro) if intro
                       else desc_meta)
        ...
        video_src = self._search_regex(
            r've" id="tabslist"><a[^>]+>(.+)</a></li>', webpage, 'video_src', default='0')
        video_url = self._search_regex(
            r"video:\s*{\s*url:\s*'(.+)',\s*type:", webpage, 'video_url', default=None)
        if url_or_none(video_url):
        ...
        for x in other_src:
            ...
            video_url = self._search_regex(
                r"video:\s*{\s*url:\s*'(.+)',\s*type:", page, 'video_url', default=None)
            if url_or_none(video_url):
...
class GimyDetailIE(GimyIE):
    ...
    _TESTS = [{
        ...
        'info_dict': {
            'id': '100699',
            'title': '工作細胞black',
            'description': r're:《工作細胞black》 已完結',
            'thumbnail': r're:https?://',
            'release_year': 2021,
            'categories': ['動漫'],
            'location': '日本',
    ...
    def _real_extract(self, url):
        ...
        # extract video info
        title = (self._html_search_regex(
                    r'h1 class="title">(.+)<!--<span class="score', webpage, 'title', default=None)
                 or self._html_extract_title(webpage).split(' - ')[0])
        desc_meta = self._html_search_meta(
            ['description', 'og:description', 'twitter:description'], webpage, 'description', default=None)
        intro = self._html_search_regex(
        ...
        release_year = int_or_none(
            self._html_search_regex(r'年份：</span><[^>]+>(\d+)</a>', webpage, 'release_year', default=None))
        ...
        if playlist:
            entries = entries + [self.url_result('https://gimy.cc' + x[0], GimyIE)
