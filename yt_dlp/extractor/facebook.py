...
class FacebookIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if post_data and not re.search(r'"[^"]+[^(feed)]_story[^"]*":', ','.join(post_data)):
            raise ExtractorError('An error occurred while downloading webpage. Please try again.', expected=True)
    ...
    def _real_extract(self, url):
        ...
        for idx, entry in enumerate(traverse_obj(
            ...
            entries.append({
                ...
                'formats': self._extract_formats(entry, idx),
            })
