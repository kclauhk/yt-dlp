...
class FacebookIE(InfoExtractor):
    ...
    _TESTS = [{
        ...
        # linked url: https://www.facebook.com/attn/videos/1569199726448814/
        # data.video.story.attachments[].media
        ...
        # data.node.comet_sections.content.story.attachments[].style.attachment.media
    ...
    def _real_extract(self, url):
        ...
        def find_json_obj(json_strings, *patterns, obj_in_value=False, get_all=False):
            ...
            for json_str in (json_strings if isinstance(json_strings, list) else [json_strings]):   # loop all
                if isinstance(json_str, str):
                    # check if json_str is a JSON string and get the quotation mark (either " or ')
                    if quote := (lambda x: x.group(1) if x else None)(re.search(r'(["\']):\s*[\[{]*\1', json_str)):
                        for patterns_item in patterns:
                            for pattern in (patterns_item if isinstance(patterns_item, tuple) else [patterns_item]):
        ...
        if len(og_title) > 1 and re.search(r'\s(?:reactions|shares|views)\s', og_title[0]):
            og_title.pop(0)
        og_title = re.sub(r'(\s*\n\s*)', ' ', ' | '.join(og_title))
...
class FacebookAdsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if post_data := [self._parse_json(j, video_id, fatal=False)
            ...
            data = traverse_obj(post_data, (..., 'require', ..., ..., ..., '__bbox',
                'require', ..., ..., ..., 'entryPointRoot', 'otherProps', 'deeplinkAdCard', 'snapshot', {dict}), get_all=False)
        if not data:
