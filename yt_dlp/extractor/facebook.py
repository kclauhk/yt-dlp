    ...
    def _real_extract(self, url):
        ...
        def extract_metadata(field=None):
            ...
            if field in ('title', 'description', None):
                for x in find_json_obj(
                        post_data, rf'{Q}message{Q}:(?:(?!{Q}message{Q}:)[^}}])+{Q}text{Q}:\s*{Q}[^{Q}](?:(?!{Q}id{Q}:).)+{Q}id{Q}:',
                        get_all=True):
            ...
            if field in ('comment_count', None):
                for x in find_json_obj(feedback_data, (rf'comments{Q}:\s*[^}}]+(total_count{Q}:\s*\d+)',
                    ...
                    webpage_info['comment_count'] = parse_count(traverse_obj(
                        json.loads(x[1]), (('total_count', (..., 'ig_comment_count'), 'total_comment_count'), {str_or_none}),
                        get_all=False))
            # share count
        ...
        if linked_url:
            entries.append(self.url_result(
                linked_url, video_id=video_id, url_transparent=url_transparent,
                **{k: v for k, v in (extract_metadata() if url_transparent else {}).items() if v}))
            if f'{Q}dash_manifest_url' not in data:
                return entries[0]
