...
class FacebookIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        ), re.sub(r'<title>(Facebook(\sLive)?)|(Video)</title>', '',
        ...
        def extract_dash_manifest(video, formats=[], subtitles={}):
            dash_manifest = video.get('playlist')
            if dash_manifest:
            ...
            def parse_graphql_video(video):
                ...
                for key, format_id in (('playable_url', 'sd'), ('playable_url_quality_hd', 'hd'),
                    ...
                    if determine_ext(playable_url) == 'mpd':
                        ...
                        formats.extend(dash_fmts)
                        if is_broadcast:
                            captions = self._merge_subtitles(captions, dash_subs)
                        else:
                            subtitles = self._merge_subtitles(subtitles, dash_subs)
                    else:
                ...
                info = {
                    ...
                    'title': ((v_title if video.get('name') else None)
                              or (f"{webpage_info['title']} Facebook Video #{v_id}" if webpage_info['title']
                                  else (f"{uploader}Facebook Video #{v_id}" if uploader else f'Facebook Video #{v_id}'))),
                    'description': v_desc or webpage_info['description'],
                    'thumbnails': thumbnails,
                    'timestamp': v_timestamp or webpage_info['timestamp'],
                    'uploader': uploader,
