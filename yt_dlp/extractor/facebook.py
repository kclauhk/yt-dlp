...
class FacebookIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if post_data and '_story' not in ','.join(post_data):
        ...
        if data:
            ...
            def parse_graphql_video(video):
                ...
                if not description and (v_id != video_id):
                    if vs_id := traverse_obj(video, (
                        ...
                        if x := list(find_json_obj(
                            ...
                            description = (lambda x: x if x != uploader else None
                                           )(json.loads(x[0][1])['message']['text'])
                    else:
                        for x in find_json_obj(data, r'"video":\s?{"id":\s?"%s"' % v_id, get_all=True):
                            if description := try_get(json.loads(x[1]), lambda x: x['message']['text']):
                                break
                # title
                ...
                info = {
                    ...
                    'description': description or webpage_info['description'],
                    'thumbnails': thumbnails,
                    'timestamp': v_timestamp or timestamp,
                    'uploader': uploader,
