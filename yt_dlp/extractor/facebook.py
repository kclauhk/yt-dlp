...
class FacebookIE(InfoExtractor):
    ...
    def _extract_from_url(self, url, video_id):
        ...
        if post_data and '"__typename":' not in webpage:
            self._sleep(5, video_id, '%(video_id)s: Retry downloading webpage after %(timeout)s seconds')
            webpage = self._download_webpage(
                re.sub(r'://(?:[\w-]+\.)?facebook\.com/', '://en-us.facebook.com/', url), video_id)
        ...
        for p_data in post_data[:]:
            if '"feed_unit":' not in p_data:
                ...
                if not data:
                    if '"dash_manifest_url":' in p_data[:p_data.find('"comment_list_renderer":{')]:
