...
class FacebookIE(InfoExtractor):
    ...
    def _extract_from_url(self, url, video_id):
        ...
        ), re.sub(r'<title>(Facebook(\sLive)?)|(Video)</title>', '', webpage), 'title', default=None, group='content')
        ...
        title = (title or info_json_ld.get('title') or page_title or '').split(' | ')[0]
            ...
            if '"feed_unit":' not in p and '"dash_manifest_url":' in p:
                for x in find_json_obj(p, r'"data":\s?{"[^"]*":', r'"data":', obj_in_value=True):
                    if '"dash_manifest_url":' in x[1]:
                        ...
                        if not p_id:
                            if not s_id:
                                p_id = traverse_obj(x_dict, (('attachments', 'node', ...), 'post_id', {str_or_none}), get_all=False)
                                s_id = traverse_obj(x_dict, (('attachments', 'node', ...), 'id', {str_or_none}), get_all=False)
                            elif s_id.isnumeric():
                                p_id = s_id
        ...
        # description
        for x in find_json_obj(','.join(post_data), (
            ...
            for i in [i for i in [s_id, p_id] if i is not None]:
                if x_dict.get('id') == i:
                    if description := x_dict['message'] if isinstance(x_dict['message'], str) else (x_dict['message'] or {}).get('text'):
                        break
            if description:
                break
        # title / description
        if not title or (not description and not page_description):
            for x in find_json_obj(','.join(post_data), r'"title":[^}]*"text":\s?"', r'"title":', get_all=True):
                ...
                for i in [i for i in [s_id, p_id] if i is not None]:
                    if x_dict.get('id') == i:
                        if text := x_dict['title'] if isinstance(x_dict['title'], str) else (x_dict['title'] or {}).get('text'):
                            title = title if title else text
                            page_description = page_description if page_description else text
                            break
        ...
        webpage_info = {
            'title': title or (description or page_description or '').split("\n")[0],
            ...
            if data:
                ...
                def parse_graphql_video(video):
                    ...
                    if uploader_id := traverse_obj(video, ('owner', 'id', {str_or_none})):
                        if x := list(find_json_obj(','.join(post_data), (
                            r'"id":\s?"%s"[^}]*"name":' % uploader_id, r'"name":[^}]*"id":\s?"%s"' % uploader_id
                        ))):
                            if x[0][1]:
                                video['owner'] = merge_dicts(video['owner'], json.loads(x[0][1]))
                    elif x := list(find_json_obj(','.join(post_data), (
                        r'(_creator":)[^}]*"name":"', r'([_"]owner":)[^}]*"name":"', r'("actor":)[^}]*"name":"'
                    ), obj_in_value=True)):
                        if x[0][1]:
                            video['owner'] = json.loads(x[0][1])
                    if webpage_info['title'] == video.get('owner', {}).get('name'):
                        webpage_info['title'] = page_title

                    info = {
                        'id': v_id,
                        'title': video.get('name') or f"{(lambda x: x if x else 'Facebook video')(webpage_info['title'])} #{v_id}",
                        'description': try_get(video, lambda x: x['savable_description']['text']) or webpage_info['description'],
                ...
                if 'Facebook video #' in video_info.get('title', '') and (title or video_info['uploader']):
                    video_info['title'] = title or video_info['uploader']
...
class FacebookRedirectURLIE(InfoExtractor):
    ...
    _TESTS = [{
        ...
        'info_dict': {
            ...
            'availability': 'public',
            'uploader_url': 'https://www.youtube.com/@boilerroom',
