    ...
    def _real_extract(self, url):
                    ...
                    if re.search(rf'{Q}attachment{Q}:\s*{{{Q}(?:source|web_link){Q}:', p_data):
                        ...
                        for x in find_json_obj(p_data, rf'{Q}attachment{Q}:\s*{{{Q}(?:source|web_link){Q}:',
                            ...
                            if link_url := traverse_obj(json.loads(x[1]), (
                                ...
                                data = True
                                break
                    p_data_sliced = p_data[:p_data.find(f'{Q}comment_list_renderer{Q}:')]
        ...
        if isinstance(data, str):
            ...
            if 'codecs=\\"avc' not in data:
                if any(k in self._downloader.params for k in ['cookiefile', 'cookiesfrombrowser']):
                    if self.cookiejar.filename:
                        self.cookiejar.save()
                    # discard cookies
                    for k in ['cookiefile', 'cookiesfrombrowser']:
                        self._downloader.params[k] = None
