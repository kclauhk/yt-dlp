...
class FacebookIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        def extract_dash_manifest(video, formats=[], subtitles={}):
            ...
            dash_manifest_url = traverse_obj(video, ('videoDeliveryLegacyFields', 'dash_manifest_url', {str}))
            if dash_manifest and dash_manifest_url:
                dash_fmts, dash_subs = self._parse_mpd_formats_and_subtitles(
                    compat_etree_fromstring(urllib.parse.unquote_plus(dash_manifest)),
                    mpd_url=dash_manifest_url)
        ...
        if data:
            def parse_graphql_video(video):
                ...
                for key, format_id in (('playable_url', 'sd'), ('playable_url_quality_hd', 'hd'),
                    ...
                    playable_url = video['videoDeliveryLegacyFields'].get(key)
                    if not playable_url:
                        continue
