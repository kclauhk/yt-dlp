...
class FacebookIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        def extract_dash_manifest(video, formats=[], subtitles={}):
            if dash_manifest := traverse_obj(video, 'dash_manifest_xml_string', 'dash_manifest',
                                             ('dash_manifests', ..., 'manifest_xml'), 'playlist',
                                             expected_type=str):
                dash_manifest_url = variadic(traverse_obj(
                    video, 'dash_manifest_url', ('dash_manifest_urls', ..., 'manifest_url'), {url_or_none}))
                for i, manifest in enumerate(variadic(dash_manifest)):
                    fmts, subs = self._parse_mpd_formats_and_subtitles(
                        compat_etree_fromstring(urllib.parse.unquote_plus(manifest)),
                        mpd_url=dash_manifest_url[i])
                    formats.extend(fmts)
                    self._merge_subtitles(subs, target=subtitles)
            return formats, subtitles

        def extract_hls_playlist(video, formats=[], subtitles={}):
            if playlist_url := traverse_obj(video, ('hls_playlist_urls', (None, ...),
                                                    'hls_playlist_url', {url_or_none})):
                for m3u8_url in variadic(playlist_url):
        ...
        if data:
            def parse_graphql_video(video):
                ...
                fmt_data = traverse_obj(video, (('videoDeliveryLegacyFields',
                                                 ('videoDeliveryResponseFragment', 'videoDeliveryResponseResult')),
                                                {dict}), get_all=False) or video
                for key, format_id in (('browser_native_hd_url', 'hd'), ('browser_native_sd_url', 'sd'),
                                       ('progressive_url', 'hd'), ('progressive_url', 'sd')):
                        # obsolete key ('playable_url', 'sd'), ('playable_url_quality_hd', 'hd'), ('playable_url_dash', '') removed
                    playable_url = traverse_obj(fmt_data, ((None,
                                                            ('progressive_urls', lambda _, v: v['metadata']['quality'] == format_id.upper())),
                                                           key, {url_or_none}), get_all=False)
                    if not playable_url:
                        continue
                ...
                if is_broadcast:
                    formats, captions = extract_hls_playlist(fmt_data, formats, captions)
                    formats, captions = extract_dash_manifest(fmt_data, formats, captions)
                else:
                    formats, subtitles = extract_hls_playlist(fmt_data, formats, subtitles)
                    formats, subtitles = extract_dash_manifest(fmt_data, formats, subtitles)
