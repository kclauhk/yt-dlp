...
class FacebookIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for p_data in post_data[:]:
            if rf'{Q}feed_unit{Q}:' in p_data or not re.search(rf'{Q}(?:dash_manifest_url|message|event_description){Q}:', p_data):
        ...
        if data:
            def parse_graphql_video(video):
                ...
                elif x := list(find_json_obj(post_data, (rf'(_creator{Q}:)[^}}]*{Q}name{Q}:\s*{Q}[^{Q}]',
                                                         rf'(owner{Q}:)[^}}]*{Q}name{Q}:\s*{Q}[^{Q}]'),
                                             obj_in_value=True)):
                    if x[0][1]:
