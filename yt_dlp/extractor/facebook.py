    ...
    def _real_extract(self, url):
        ...
        def extract_metadata(field=None):
            ...
            if field in ('timestamp', None):
                for x in find_json_obj(post_data, rf'creation_time{Q}:\s*\d+,',
                                       rf'created_time{Q}:\s*\d+,',
                                       rf'publish_time{Q}:\s*\d+,', get_all=True):
                    if re.search(rf'id{Q}:\s*{Q}(?:(?:{s_id})|(?:{p_id})){Q}', x[1]):
                        post_info['timestamp'] = json.loads(x[1]).get(re.split(f'{Q}', x[0])[0])
                        break
            # view count
            if field in ('view_count', None):
                post_info['view_count'] = parse_count(self._search_regex(
                    (rf'play_count{Q}\s*:\s*(\d+)',
                     rf'video_view_count{Q}\s*:\s*(\d+)',
            ...
            if field in ('like_count', None):
                post_info['like_count'] = parse_count(self._search_regex(
                    (rf'localized_name{Q}:\s*{Q}Like{Q}.[^}}]+{Q}reaction_count{Q}:\s*(\d+)}}',
                     rf'likers{Q}:\s*{{{Q}count{Q}:\s*(\d+)}}',
                     ), reaction_data, 'like count', default=None))
            # comment count
            if field in ('comment_count', None):
                for x in find_json_obj(reaction_data, (rf'comments{Q}:\s*[^}}]+(total_count{Q}:\s*\d+)',
            ...
            if field in ('repost_count', None):
                for x in find_json_obj(reaction_data, (rf'share_count{Q}:\s*[^}}]+(count{Q}:\s*"?\d+)',
        ...
        Q = self._search_regex(r'(["\']):\s*[\[{]*\1', (post_data[0] if post_data else ''),
                               'quotation', default='"')
        for p_data in post_data[:]:
            if (any(k in p_data for k in [rf'{Q}feed_unit{Q}:', rf'{Q}news_feed{Q}:',
                                          rf'{Q}feed_backend_data{Q}:'])
                ...
                if ((not s_id or not p_id)
                        and any(k in p_data for k in [f'{Q}story{Q}:', f'{Q}creation_story{Q}:'])):
                    p_id = p_id or self._search_regex(
                        rf'{Q}(?:post_id|videoId|video_id){Q}:\s*{Q}(\d+){Q}', p_data,
                        'post id', default=(video_id if video_id.isnumeric() else None))
                    s_id = s_id or self._search_regex(
                        rf'id{Q}:\s*{Q}(Uzpf[^{Q}]+){Q}', p_data, 'story id', default=None)
                if not data:
                    if re.search(rf'{Q}attachment{Q}:\s*{{{Q}(?:source|web_link){Q}:', p_data):
                        ...
                        for x in find_json_obj(p_data, rf'{Q}attachment{Q}:\s*{{{Q}(?:source|web_link){Q}:',
                            ...
                            if link_url := traverse_obj(json.loads(x[1]), (
                                ...
                                for y in find_json_obj(p_data[:p_data.find(x[1])],
                                                       rf'{Q}attachment{Q}:\s*{{{Q}media{Q}:\s*{{',
                                                       obj_in_value=True):
                                ...
                                break
                    comment_pos = p_data.find(f'{Q}comment_list_renderer{Q}:')
                    p_data_sliced = p_data if comment_pos == -1 else p_data[:comment_pos]
                    if f'{Q}dash_manifest_url' in p_data_sliced:
                        for x in find_json_obj(p_data, rf'{Q}data{Q}:\s*{{', rf'{Q}data{Q}:',
                                               obj_in_value=True):
                            if f'{Q}dash_manifest_url' in x[1]:
                                data = x[1]
                                break
                    if f'{Q}post_level_music_info{Q}' in p_data_sliced:
                        for x in find_json_obj(p_data, rf'{Q}data{Q}:\s*{{', rf'{Q}data{Q}:',
                                               obj_in_value=True):
                            if f'{Q}audio_encodings{Q}' in x[1]:
                                data = x[1]
                                break
            if (not reaction_data
                    and any(k in p_data for k in [f'{Q}likers{Q}:', f'{Q}Like{Q}}},',
                                                  f'comment_count{Q}:'])):
                for x in find_json_obj(p_data, rf'fb_reel_react_button{Q}:\s*{{',
                                       rf'reaction_count{Q}:\s*{{', rf'feedback{Q}:\s*{{'):
        ...
        if isinstance(data, str):
            def parse_graphql_video(video, video_avc=None):
                ...
                if not v_timestamp and v_id != video_id:
                    for x in find_json_obj(post_data, rf'creation_time{Q}:\s*\d+,',
                                           rf'created_time{Q}:\s*\d+,', rf'publish_time{Q}:\s*\d+,',
                                           get_all=True):
                ...
                if uploader_id := traverse_obj(video, ('owner', 'id', {str_or_none})):
                    if x := list(find_json_obj(data, (
                            rf'id{Q}:\s*{Q}{uploader_id}{Q}[^}}]*{Q}name{Q}:\s*{Q}[^{Q}]',
                ...
                if not v_desc and v_id != video_id:
                    ...
                    # else:
                    #    for x in find_json_obj(
                    #            data, rf'video{Q}:\s*{{{Q}id{Q}:\s*{Q}{v_id}{Q}', get_all=True):
            ...
            video_data = traverse_obj(
                list(find_json_obj(data, (rf'videoDeliveryLegacyFields{Q}:',
                                          rf'post_level_music_info{Q}:'), get_all=True)),
