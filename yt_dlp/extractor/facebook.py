...
class FacebookIE(InfoExtractor):
    ...
    _TESTS = [{
        ...
        'info_dict': {
            'id': '1072691702860471',
            'ext': 'mp4',
            'title': 'md5:23f35f9da40471e42b262c6b9574c098',
    ...
    def _extract_from_url(self, url, video_id):
        ...
        data = []
        p_id = s_id = linked_url = description = uploader_info = None
        ...
        if not title or (not description and not page_description):
            for x in find_json_obj(','.join(post_data), r'"title":(?:(?!"title":).)*"text":\s?"[^"]', get_all=True):
                x_dict = json.loads(x[1])
        ...
        webpage_info = {
            'title': ((title if title != (uploader_info or {}).get('uploader') else None)
                      or page_title or (description or page_description or '').split("\n")[0]),
        ...
        if linked_url:
            return self.url_result(linked_url, video_id=video_id,
                                   video_title=((title if title != (uploader_info or {}).get('uploader') else None)
                                                or page_title or (description or page_description or '').split("\n")[0]),
        ...
        if data:
            ...
            def parse_graphql_video(video):
                ...
                if webpage_info['title'] == uploader:
            ...
            if ' Video #' in video_info.get('title', '') and (title or video_info['uploader']):
