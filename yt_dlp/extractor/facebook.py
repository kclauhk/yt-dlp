...
class FacebookIE(InfoExtractor):
    ...
    _TESTS = [{
        ...
        'info_dict': {
            ...
            'duration': 3133.583,
            'thumbnail': r're:https?://scontent[^/]+\.fbcdn\.net/',
        ...
        'info_dict': {
            'id': '1022281775053849',
            'ext': 'mp4',
            'title': 'Satrio Waluyo on Reels',
            'description': '',
            'duration': 7.104,
            'thumbnail': r're:https?://scontent[^/]+\.fbcdn\.net/',
            'timestamp': 1650433944,
            'upload_date': '20220420',
            'uploader': 'Satrio Waluyo',
            'uploader_id': '100073071055552',
            'uploader_url': r're:https?://\w',
            'live_status': 'not_live',
            'concurrent_view_count': int,
            'view_count': int,
    ...
    _WEBPAGE_TESTS = [{
        # <iframe> embed
        # formats: progressive_sd_src, progressive_hd_src
    ...
    _SUPPORTED_PAGELETS_REGEX = r'(?:pagelet_group_mall|permalink_video_pagelet|hyperfeed_story_id_[0-9a-f]+)'
    _api_config = {
    ...
    def _perform_login(self, username, password):
        ...
        try:
            ...
            self.report_warning(f'unable to log in: {e.cause.reason}')
            return
    ...
    def _real_extract(self, url):
        ...
        for retry in self.RetryManager():
            if retry.attempt > 1 and not self.get_param('retry_sleep_functions', {}).get('extractor'):
                self._sleep(3, video_id, msg_template='Sleeping 3.00 seconds ...')
            try:
                ...
                props = get_first(sjs_data, (
                    'require', ..., ..., ..., '__bbox', 'require', ..., ..., ..., (None, (..., ...)),
                    'rootView', lambda _, v: v.get('title') is not None))
                login_state = get_first(sjs_data, (
                    'require', ..., ..., ..., '__bbox', 'require', ..., ..., ..., '__bbox', 'result',
                    'data', (('ufac_client', 'state', (('set_contact_point_state_renderer', 'title'),
                                                       ('intro_state_renderer', 'header_title'))),
                             ('epsilon_checkpoint', 'screen', 'title'))))
                if (post_data and not props and not login_state
                        and not re.search(r'"[^"]+[^(feed)]_story[^"]*":', ','.join(post_data))):
                    raise ExtractorError('Redirected to login page.', expected=True)
            except ExtractorError as e:
                if not login_data and retry.attempt == 2:
        ...
        if login_data:
            ...
            if logged_in and login_state:
                if any(content in login_state for content in ['days left to appeal', 'suspended your account']):
                    raise ExtractorError('Your account is suspended', expected=True)
                if 'Enter mobile number' == login_state:
                    raise ExtractorError('Facebook is requiring mobile number confirmation', expected=True)
                if 'your account has been locked' in login_state:
        ...
        def find_json_obj(json_strings, *patterns, obj_in_value=False, get_all=False):
            ...
            @param  json_strings    string/list     JSON string/a list of JSON strings (match all)
                    *patterns       string/tuple    regex patterns (if tuple, return only the 1st matched pattern)
                    obj_in_value    boolean         False:  find the object(s) containing (one of) the pattern(s)
                                                    True :  find the object(s) in the value of key(s) that match
                                                            (one of) the pattern(s)
            ...
            """
            def find_offset(string, bracket, quot):
                _BRACKET_MAP = {
                    # '{': ([f'{{{quot}'], ['},', '}]', '}}', f'}}{quot}'], (1 if obj_in_value else -1)),
                    # '}': (['},', '}]', '}}', f'}}{quot}'], [f'{{{quot}'], 1),
                    '{': ([f'{{{quot}', '{\\'], ['},', '}]', '}}', f'}}{quot}'], (1 if obj_in_value else -1)),
                    '}': (['},', '}]', '}}', f'}}{quot}'], [f'{{{quot}', '{\\'], 1),
                }      # ([search pattern], [opposite sign], search direction); search direction: 1: forward, -1: backward
                # string = re.sub(rf'{{\\{quot}([^{quot}]+\\{quot}:)', rf'{{{quot}\1 ', string.replace('{}', '[]'))
                string = string.replace('{}', '[]')  # remove empty objects
                count, b_sum, offset = 0, 0, 0
                direction = _BRACKET_MAP[bracket][2]
                for y, x in zip(((string[1:] + ' ') if direction > 0 else (' ' + string[:-1]))[::direction],
                                string[::direction], strict=False):
                    s = (x + y) if direction > 0 else (y + x)
                    count += (1 if s in _BRACKET_MAP[bracket][0] or s in _BRACKET_MAP[bracket][1] else 0)
                    b_sum += (1 if s in _BRACKET_MAP[bracket][0]
                              else (-1 if s in _BRACKET_MAP[bracket][1] else 0))
                    offset += 1
            ...
            for json_str in variadic(json_strings):   # loop all
                if isinstance(json_str, str):
                    ...
                    if quot := self._search_regex(r'(["\']):\s*[\[{]*\1', json_str, 'quot', default=None):
                            ...
                            for pattern in variadic(patterns_item):
                                ...
                                if isinstance(pattern, str):
                                    for m in re.finditer(pattern, json_str):    # break according to get_all
                                        if obj_in_value:
                                            i = (lambda x, y: (m.start(m.lastindex or 0) + x - 1) if x > 0  # noqa: PLC3002
                                        ...
                                        if i is not None:
                                            ...
                                            closing = i + find_offset(json_str[i:], '}', quot)
        ...
        def truncate_string(string, left, right=0, threshold=0):
            assert left > 3 and right >= 0 and threshold >= 0
            if string is None or len(string) <= max(left + right, threshold):
                return string
            pos = left - 3
            return f'{string[:pos + string[pos:pos + 20].rfind(" ")]}...{string[-right:] if right else ""}'

        def extract_metadata(field=None):
            if post_info.get(field) is not None:
                return post_info[field]
            elif field is None and post_info.get('timestamp') is not None:
                return post_info
            # extract data
            description = title = uploader = None
            # uploader
            if field == 'uploader':
                for x in find_json_obj(post_data, (r'actors":[^}]+"__isActor":',
                                                   r'owner":[^}]+"name":\s*"[^"]'),
                                       get_all=True):
                    if re.search(rf'id":\s*"(?:{s_id}|{video_id})"', x[1]):
                        uploader = traverse_obj(json.loads(x[1]), {
            ...
            if field in ('title', 'description', None):
                for x in find_json_obj(
                    ...
                    if (traverse_obj(x_dict, ('message', '__typename', {str_or_none})) != 'TextWithEntities'):
                        for i in [i for i in [s_id, p_id] if i is not None]:
                            if x_dict.get('id') == i:
                                if (description := x_dict['message'] if isinstance(x_dict['message'], str)
                                    ...
                                    if track_title := self._search_regex(
                                            r'("track_title":\s*"(?:(?:[^"\\]|\\.)*)")',
                ...
                for x in find_json_obj(post_data, r'title":\s*[^}]+"text":\s*"[^"]', get_all=True):
                    x_dict = json.loads(x[1])
                    if p_id:
                    ...
                    if title and description:
                        break
                title = (lambda x: x.replace('\n', '').replace('\r', '') if x != extract_metadata('uploader') else None  # noqa: PLC3002
                         )(title
                           or (self._html_search_regex(
                               ...
                               or (lambda x: '' if not x or x.group(1) in ('Video', 'Facebook', 'Facebook Live')    # noqa: PLC3002
                                   else x.group(1).encode().decode('unicode_escape')
                                   )(re.search(r'"meta":\s*{"title":\s*"((?:[^"\\]|\\.)*)"', webpage))
                               or og_title
                               ).split(' | ')[0]
                           or re.sub(r'(\s*\n\s*)', ' ', description or ''))
                post_info['title'] = truncate_string(title, 50, threshold=100)
                post_info['description'] = description
            # timestamp
            if field in ('timestamp', None):
                for x in find_json_obj(post_data, r'creation_time":\s*\d+,',
                                       r'created_time":\s*\d+,',
                                       r'publish_time":\s*\d+,', get_all=True):
                    if re.search(rf'id":\s*"(?:(?:{s_id})|(?:{p_id}))"', x[1]):
                        post_info['timestamp'] = json.loads(x[1]).get(re.split('"', x[0])[0])
                        break
            # view count
            if field in ('view_count', None):
                post_info['view_count'] = parse_count(self._search_regex(
                    (r'play_count"\s*:\s*(\d+)',
                     r'video_view_count"\s*:\s*(\d+)',
                     ), reaction_data, 'video view count', default=None))
            # like count
            if field in ('like_count', None):
                post_info['like_count'] = parse_count(self._search_regex(
                    (r'localized_name":\s*"Like".[^}]+"reaction_count":\s*(\d+)}',
                     r'likers":\s*{"count":\s*(\d+)}',
                     ), reaction_data, 'like count', default=None))
            # comment count
            if field in ('comment_count', None):
                for x in find_json_obj(reaction_data, (r'comments":\s*[^}]+(total_count":\s*\d+)',
                                                       r'total_comment_count":\s*\d+')):
                    post_info['comment_count'] = parse_count(traverse_obj(
                        json.loads(x[1]),
                        (('total_count', (..., 'ig_comment_count'), 'total_comment_count'), {str_or_none}),
                        get_all=False))
            # share count
            if field in ('repost_count', None):
                for x in find_json_obj(reaction_data, (r'share_count":\s*[^}]+(count":\s*"?\d+)',
                                                       r'share_count(?:_reduced)":\s*"?\d+')):
                    post_info['repost_count'] = parse_count(traverse_obj(
                        json.loads(x[1]), (('count', 'share_count_reduced', 'total_count'), {str_or_none}),
                        get_all=False))
            # return post_info
            return post_info.get(field) if field else post_info
        ...
        post_info = {
            ...
            'view_count': parse_count(self._search_regex(
                r'\bviewCount\s*:\s*["\']([\d,.]+)', webpage, 'view count', default=None)),
        }
        ...
        # real extract
        p_id, s_id, web_links, entries, data, reaction_data = None, None, [], [], None, ''
        for p_data in post_data[:]:
            json_str = json.dumps(json.loads(js_to_json(p_data)))
            if (any(k in json_str for k in [r'"feed_unit":', r'"news_feed":',
                                            r'"feed_backend_data":'])
                    or not re.search(r'"(?:dash_manifest_urls?|message|event_description)":', json_str)):
                # discard feed data
                post_data.remove(p_data)
            else:
                if ((not s_id or not p_id)
                        and any(k in json_str for k in ['"story":', '"creation_story":'])):
                    p_id = p_id or self._search_regex(
                        r'"(?:post_id|videoId|video_id)":\s*"(\d+)"', json_str,
                        'post id', default=(video_id if video_id.isnumeric() else None))
                    s_id = s_id or self._search_regex(
                        r'id":\s*"(Uzpf[^"]+)"', json_str, 'story id', default=None)
                if not data:
                    if re.search(r'"attachment":\s*{"(?:source|web_link)":', json_str):
                        # linked video
                        for x in find_json_obj(json_str, r'"attachment":\s*{"(?:source|web_link)":',
                                               obj_in_value=True):
                            if link_url := traverse_obj(json.loads(x[1]), (
                                    ('web_link', None), 'url', {url_or_none}), get_all=False):
                                web_links.append({
                                    'url': link_url,
                                    'transparent': '.facebook.com' not in urllib.parse.urlparse(link_url).netloc,
                                })
                                for y in find_json_obj(json_str[:json_str.find(x[1])],
                                                       r'"attachment":\s*{"media":\s*{',
                                                       obj_in_value=True):
                                    if share_image := traverse_obj(
                                            json.loads(y[1]), ('large_share_image'), get_all=False):
                                        post_info['thumbnails'].append(traverse_obj(share_image, {
                                            'url': ('uri', {url_or_none}),
                                            'height': ('height', {int_or_none}),
                                            'width': ('width', {int_or_none}),
                                        }))
                                        break
                                data = {'type': 'linked_video'}
                                break
                    c_pos = json_str.find('"comment_list_renderer":')
                    json_sliced = json_str if c_pos == -1 else json_str[:c_pos]
                    if '"dash_manifest_url' in json_sliced:
                        for x in find_json_obj(json_str, r'"data":\s*{', r'"data":',
                                               obj_in_value=True):
                            if '"dash_manifest_url' in x[1]:
                                data = x[1]
                                break
                    if '"post_level_music_info":' in json_sliced:
                        for x in find_json_obj(json_str, r'"data":\s*{', r'"data":',
                                               obj_in_value=True):
                            if '"audio_encodings":' in x[1]:
                                data = x[1]
                                break
                post_data[post_data.index(p_data)] = json_str
            if (not reaction_data
                    and any(k in json_str for k in ['"likers":', '"Like"},',
                                                    'comment_count":'])):
                for x in find_json_obj(json_str, r'fb_reel_react_button":\s*{',
                                       r'reaction_count":\s*{', r'feedback":\s*{'):
        ...
        if isinstance(data, str):
            def parse_graphql_video(video, video_avc=None):
                get_mpd_formats = self._extract_mpd_formats_and_subtitles
                v_id = video.get('videoId') or video.get('id') or video_id
                q = qualities(['sd', 'hd'])
                formats, captions, subtitles = [], {}, {}
                is_broadcast = traverse_obj(video, ('is_video_broadcast', {bool}))
                cap_or_subs = captions if is_broadcast else subtitles

                # videoDeliveryLegacy formats extraction
                fmt_data = traverse_obj(video, ('videoDeliveryLegacyFields', {dict})) or video
                for key, format_id in (('browser_native_hd_url', 'hd'),
                                       ('browser_native_sd_url', 'sd')):
                    # obsoleted: ('playable_url', 'sd'), ('playable_url_quality_hd', 'hd'), ('playable_url_dash', '')
                    if playable_url := fmt_data.get(key):
                        if determine_ext(playable_url) == 'mpd':
                            fmts, subs = get_mpd_formats(playable_url, video_id, fatal=False)
                            formats.extend(fmts)
                            self._merge_subtitles(subs, target=(cap_or_subs))
                        else:
                            q = qualities(['sd', 'hd'])
                            formats.append({
                                'format_id': format_id,
                ...
                extract_dash_manifest(fmt_data, formats, [cap_or_subs])
                if video_avc:
                    extract_dash_manifest(
                        traverse_obj(video_avc, ('videoDeliveryLegacyFields', {dict})),
                        formats, [cap_or_subs])
                # videoDeliveryResponse formats extraction
                if fmt_data := traverse_obj(video, ('videoDeliveryResponseFragment',
                                                    'videoDeliveryResponseResult')):
                    mpd_urls = traverse_obj(
                        fmt_data, ('dash_manifest_urls', ..., 'manifest_url', {url_or_none}))
                    dash_manifests = traverse_obj(
                        fmt_data, ('dash_manifests', lambda _, v: v['manifest_xml']))
                    for idx, dash_manifest in enumerate(dash_manifests):
                        extract_dash_manifest(dash_manifest, formats, [cap_or_subs],
                                              mpd_url=traverse_obj(mpd_urls, idx))
                    if not dash_manifests:
                        # Only extract from MPD URLs if the manifests are not already provided
                        for mpd_url in mpd_urls:
                            fmts, subs = get_mpd_formats(mpd_url, video_id, fatal=False)
                            formats.extend(fmts)
                            self._merge_subtitles(subs, target=(cap_or_subs))
                    for prog_fmt in traverse_obj(fmt_data, (
                        'progressive_urls', lambda _, v: v['progressive_url'],
                    )):
                    ...
                    for m3u8_url in traverse_obj(fmt_data, (
                        'hls_playlist_urls', ..., 'hls_playlist_url', {url_or_none},
                    )):
                        fmts, subs = self._extract_m3u8_formats_and_subtitles(
                            m3u8_url, video_id, 'mp4', fatal=False, m3u8_id='hls')
                        formats.extend(fmts)
                        self._merge_subtitles(subs, target=(cap_or_subs))
                # audio_asset extraction
                if fmt_data := video.get('audio'):
                    if fmt := traverse_obj(
                            fmt_data, ('post_level_music_info', lambda _, v: v['dash_manifest']),
                            get_all=False):
                        extract_dash_manifest(fmt, formats, [{}])
                    formats.append({
                        'format_id': 'audio',
                        'vcodec': 'none',
                        **traverse_obj(fmt_data, (
                            ('post_level_music_info', 'story_media_metadata'), {
                                'url': ((('audio_encodings', ..., 'url'), 'audio_url'),
                                        {url_or_none}),
                            }), get_all=False),
                    })
                ...
                for caption in traverse_obj(video, (
                    ...
                    subs = {
                        ...
                        'name': format_field(caption, 'localized_country', f'{lang} (%s)',
                                             default=lang),
                    }
                ...
                if captions_url and not captions and not subtitles:
                    ...
                    (cap_or_subs)[locale] = [{'url': captions_url}]
                # thumbnails
                thumbnails = []
                for url in [uri for uri in [traverse_obj(video, path) for path in [
                    ...
                    if (re.search(r'\.(?:jpg|png)', url)
                        ...
                        thumbnails.append({k: v for k, v in {
                            'url': url,
                            'height': int_or_none(self._search_regex(
                                                  r'stp=.+_[a-z]\d+x(\d+)&', url, 'thumbnail height',
                                                  default=None)),
                            'preference': None if 'stp=' in url else 1,
                        }.items() if v is not None})
                # timestamp
                v_timestamp = traverse_obj(
                    video, 'publish_time', 'creation_time', 'created_time', {int_or_none})
                if not v_timestamp and v_id != video_id:
                    for x in find_json_obj(post_data, r'creation_time":\s*\d+,',
                                           r'created_time":\s*\d+,', r'publish_time":\s*\d+,',
                                           get_all=True):
                        if re.search(rf'id":\s*"{v_id}"', x[1]):
                            if v_timestamp := json.loads(x[1]).get(x[0].split('"')[0]):
                                break
                # uploader
                if uploader_id := traverse_obj(video, ('owner', 'id', {str_or_none})):
                    if x := list(find_json_obj(data, (
                            rf'id":\s*"{uploader_id}"[^}}]*"name":\s*"[^"]',
                            rf'"name":\s*"[^"][^}}]*"id":\s*"{uploader_id}"'))):
                        if x[0][1]:
                            video['owner'] = merge_dicts(video['owner'], json.loads(x[0][1]))
                elif x := list(find_json_obj(post_data, (
                        r'(owner":)[^}]*"name":\s*"[^"]',
                        r'((?!share)\w{5}_creator":)[^}]*"name":\s*"[^"]'),
                        obj_in_value=True)):
                    if x[0][1]:
                        video['owner'] = json.loads(x[0][1])
                        uploader_id = traverse_obj(video, ('owner', 'id', {str_or_none}))
                uploader = traverse_obj(
                    video, ('owner', 'name', {str_or_none})) or extract_metadata('uploader')
                # description
                v_desc = traverse_obj(video, ('savable_description', 'text', {str_or_none}))
                if not v_desc and v_id != video_id:
                    if vs_id := traverse_obj(video, (
                            (None, (..., 'video')), 'creation_story', 'id', {str_or_none}),
                            get_all=False):
                        if x := list(find_json_obj(
                                data, rf'"message":(?:(?!"message":)[^}}])+"text":\s*"[^"](?:(?!"id":).)+"id":\s*"{vs_id}"')):
                            v_desc = (lambda x: x if x != uploader else None    # noqa: PLC3002
                                      )(json.loads(x[0][1])['message']['text'])
                    # else:
                    #    for x in find_json_obj(
                    #            data, rf'video":\s*{"id":\s*"{v_id}"', get_all=True):
                ...
                info = {
                    ...
                    'title': (truncate_string(video.get('name'), 50, threshold=100) if video.get('name')
                                    ...
                                    else (f'{uploader} on Reels - video #{v_id}' if uploader
                                          else f'Facebook video #{v_id}'))),
                    'description': v_desc or extract_metadata('description'),
                    'thumbnails': thumbnails,
                    'timestamp': v_timestamp or extract_metadata('timestamp'),
                    'uploader': uploader,
                    'uploader_id': uploader_id or post_info.get('uploader_id'),
                    'uploader_url': (traverse_obj(video, ('owner', 'url', {url_or_none}))
                                     or (None if post_info.get('uploader') != uploader
                                         else post_info.get('uploader_url'))
                                     or (lambda x: f'https://www.facebook.com/profile.php?id={x}' if x else None    # noqa: PLC3002
                                         )(uploader_id or post_info.get('uploader_id'))),
                    ...
                    'view_count': extract_metadata('view_count'),
                    ...
                    'age_limit': 18 if (re.search(r'"validator":\s*"GRAPHIC"', data)
                                        and '"OverlayWarningScreenViewModel"' in data) else None,
                }
                process_formats(info)
                entries.append(info)

            video_ids, data_avc = [], None
            if 'codecs=\\"avc' not in data:
                ...
                data_avc = [{x.get('id') or x.get('videoId'): x} for x in [
                    json.loads(x[1]) for x in find_json_obj(
                        self._download_webpage(request, video_id),
                        (r'videoDeliveryLegacyFields":'), get_all=True)]]

            video_data = traverse_obj(
                list(find_json_obj(data, (r'videoDeliveryLegacyFields":',
                                          r'post_level_music_info":'), get_all=True)),
                (..., 1, {str_or_none}, {json.loads}))
            for idx, video in enumerate(video_data):
                if (video.get('post_level_music_info')
                        and (video.get('id') == s_id or video.get('id') == p_id)):
                    if traverse_obj(video, ((('story_media_metadata', 'audio_url'),
                                             ('post_level_music_info', 'audio_asset',
                                              'audio_encodings', ..., 'url')),
                                            {url_or_none})):
                        parse_graphql_video({
                            'audio': video,
                            'name': traverse_obj(video_data, (idx + 1, 'story_media_metadata',
                                                              'song_title', {str_or_none})),
                        })
                elif (video.get('__typename', 'Video') == 'Video'
                      and not video.get('sticker_image')
                      and video.get('id', f'{video_id}_{idx}') not in video_ids):
                    video_ids.append(video.get('id', f'{video_id}_{idx}'))
                    video_avc = traverse_obj(data_avc, (..., video.get('id'), {dict}), get_all=False)
                    parse_graphql_video(video, video_avc)
            ...
            if not video_data:
                if server_js_data := self._parse_json(self._search_regex([
                    r'bigPipe\.onPageletArrive\(({.+?})\)\s*;\s*}\s*\)\s*,\s*["\']onPageletArrive\s+' + self._SUPPORTED_PAGELETS_REGEX,
                    rf'bigPipe\.onPageletArrive\(({{.*?id\s*:\s*"{self._SUPPORTED_PAGELETS_REGEX}".*?}})\);',
            ...
            if not video_data:
                ...
                if m_msg is not None:
                    ...
                    self.raise_login_required('Login failed', method='cookies')
            ...
            for f in video_data:
                if f and isinstance(f, dict):
                    format_id = f.get('stream_type', 'progressive')
                    f = [f]
            ...
            process_formats(info)
            info.update(extract_metadata())
            entries.append(info)

        for link in web_links:
            entries.append(self.url_result(
                link['url'], video_id=video_id, url_transparent=link['transparent'],
                **{k: v for k, v in (extract_metadata() if link['transparent'] else {}).items() if v}))

        if len(entries) > 1:
            return self.playlist_result(entries, video_id, **{
                k: v for k, v in extract_metadata().items() if v})

        info_dict = entries[0] if entries else {'id': video_id}
        if info_dict.get('title'):
            info_dict['title'] = re.sub(r' - video #\d{15,}$', '', info_dict.get('title'))
        if post_info.get('thumbnails'):
            if not (any(post_info['thumbnails'][0]['url'].split('_cat=')[0] in thumbnail['url']
                        for thumbnail in info_dict.get('thumbnails', []))):
                info_dict.setdefault('thumbnails', []).extend(post_info['thumbnails'])
        return merge_dicts(info_dict, post_info)
...
class FacebookAdsIE(InfoExtractor):
    ...
    _TESTS = [{
        'url': 'https://www.facebook.com/ads/library/?id=4308539369360109',
        'info_dict': {
            'id': '4308539369360109',
            'ext': 'mp4',
            'title': 'VIDEO by Kandao',
            'description': 'md5:5d5ef9b4cab1fdbccd626aec1368772e',
            'thumbnail': r're:https?://scontent[^/]+\.fbcdn\.net/',
            'uploader': 'Kandao',
        ...
        'url': 'https://www.facebook.com/ads/library/?id=1845766693048807',
        'info_dict': {
            'id': '1845766693048807',
            'ext': 'mp4',
            'title': 'Android Studio\'s agentic experiences',
            'description': 'Build high quality apps faster with Gemini in Android Studio',
            'thumbnail': r're:https?://scontent[^/]+\.fbcdn\.net/',
            'uploader': 'Android Developers',
            'uploader_id': '1039724485895499',
            'uploader_url': r're:https?://\w',
            'like_count': int,
        },
    }, {
