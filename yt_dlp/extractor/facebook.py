    ...
    def _real_extract(self, url):
                                    ...
                                    for m in re.finditer(pattern, json_str):    # break according to get_all
                                        ...
                                        if i:
                                            opening = (i + find_offset(
                                                json_str[(i * obj_in_value):(i * (not obj_in_value) - obj_in_value * 2 + 1)], '{', quotation,
                                            ) - obj_in_value)
        ...
        def extract_metadata(field=None):
                    ...
                    for i in [i for i in [s_id, p_id] if i is not None]:
                        if x_dict.get('id') == i:
                            if (description := x_dict['message'] if isinstance(x_dict['message'], str)
                                ...
                                if track_title := self._search_regex(rf'({Q}track_title{Q}:\s*{Q}(?:(?:[^{Q}\\]|\\.)*){Q})', x[1],
                                                                     'track_title', default=None):
            ...
            if field in ('like_count', None):
                like_count = re.search(rf'likers{Q}:\s*{{{Q}count{Q}:\s*(\d+)}}', feedback_data)
            ...
            if field in ('comment_count', None):
                for x in find_json_obj(feedback_data, (rf'comments{Q}:\s*[^}}]+(total_count{Q}:\s*\d+)',
                    ...
                    webpage_info['comment_count'] = traverse_obj(
                        json.loads(x[1]), (('total_count', (..., 'ig_comment_count'), 'total_comment_count'), {int_or_none}), get_all=False)
            # share count
            if field in ('share_count', None):
                for x in find_json_obj(feedback_data, (rf'share_count{Q}:\s*[^}}]+(count{Q}:\s*"?\d+)',
                                                       rf'share_count(?:_reduced){Q}:\s*"?\d+')):
                    webpage_info['share_count'] = parse_count(
                        traverse_obj(json.loads(x[1]), (('count', 'share_count_reduced', 'total_count'), {str_or_none}), get_all=False))
            # return data
        ...
        webpage_info = {
            'thumbnails': [{k: v for k, v in {
                ...
                'height': int_or_none(self._search_regex(
                                      r'stp=.+_[a-z]\d+x(\d+)&', thumbnail, 'height', default=None)),
        ...
        for p_data in post_data[:]:
            if rf'{Q}feed_unit{Q}:' in p_data or not re.search(
                    rf'{Q}(?:dash_manifest_url|message|event_description|total_comment_count|comments){Q}:', p_data):
                # discard useless data
                post_data.remove(p_data)
            else:
                if (not s_id or not p_id) and (f'{Q}story{Q}:' in p_data or f'{Q}creation_story{Q}:' in p_data):
                    p_id = p_id if p_id else self._search_regex(rf'{Q}(?:post_id|videoId|video_id){Q}:\s*{Q}(\d+){Q}', p_data,
                                                                'p_id', default=(video_id if video_id.isnumeric() else None))
                    s_id = s_id if s_id else self._search_regex(rf'id{Q}:\s*{Q}(Uzpf[^{Q}]+){Q}', p_data, 's_id', default=None)
                if not data:
                ...
                if not feedback_data:
                    if f'{Q}total_comment_count{Q}:' in p_data or f'{Q}comments{Q}:{{' in p_data:
                        for x in find_json_obj(p_data, rf'{Q}data{Q}:\s*{{', rf'{Q}data{Q}:', obj_in_value=True):
                            if s_id in x[1] or p_id in x[1]:
                                feedback_data = x[1]
                                break
        ...
        if data:
            def parse_graphql_video(video):
                ...
                for url in [uri for uri in [traverse_obj(video, path) for path in [
                    ...
                    if (re.search(r'\.(?:jpg|png)', url)
                        ...
                        thumbnails.append({k: v for k, v in {
                            'url': url,
                            'height': int_or_none(self._search_regex(
                                                  r'stp=.+_[a-z]\d+x(\d+)&', url, 'height', default=None)),
                ...
                info = {
                    ...
                    'share_count': extract_metadata('share_count'),
                }
