    ...
    def _real_extract(self, url):
        ...
        for p_data in post_data[:]:
            if rf'{Q}feed_unit{Q}:' in p_data or not re.search(
                    rf'{Q}(?:dash_manifest_url|message|event_description){Q}:', p_data):
                    ...
                    if re.search(rf'{Q}attachment{Q}:\s*{{{Q}(?:source|web_link){Q}:', p_data):
                        ...
                        for x in find_json_obj(p_data, rf'{Q}data{Q}:\s*{{', rf'{Q}data{Q}:', obj_in_value=True):
                            if f'{Q}dash_manifest_url{Q}:' in x[1]:
                                data = x[1]
                                break
            if ((f'{Q}likers{Q}:' in p_data or f'{Q}total_comment_count{Q}:' in p_data or f'{Q}comments{Q}:{{' in p_data)
                    and not feedback_data):
                for x in find_json_obj(p_data, (rf'reaction_count{Q}:\s*{{', rf'feedback{Q}:\s*{{')):
                    if (s_id in x[1] or p_id in x[1]) and webpage.count(json.loads(x[1]).get('id', 'null')) > 1:
                        feedback_data = x[1]
                        break
...
class FacebookReelIE(InfoExtractor):
    ...
    _TESTS = [{
        ...
        'info_dict': {
            ...
            'uploader_id': '1738535909799870',  # 100040874179269
            'uploader_url': r're:^https?://.*',
            'duration': 9.579,
