...
class FacebookIE(InfoExtractor):
    ...
    _TESTS = [{
        ...
        'info_dict': {
            ...
            'thumbnail': r're:^https?://.*',
            'uploader_id': 'pfbid04yjfKy71gnEt6pQgYiJ5AT85L72YfQEQS8YSjiaX2m9uf1UBKAc7AonDnHJBDNuel',
            'duration': 131.03,
            'view_count': int,
            'concurrent_view_count': int,
        },
        ...
        'info_dict': {
            ...
            'uploader_id': 'pfbid029bJMjHJgkwJY2PEdGCJijGuMz83MszKEzdyGkJ3xv4fVHH8KBG16dd1KjKFsNHxdl',
            'concurrent_view_count': int,
            'thumbnail': r're:^https?://.*',
            'view_count': int,
            'duration': 11736.446,
        ...
        'info_dict': {
            ...
            'uploader_id': 'pfbid0oGV8JT4G6X4Sa5wgbJkULgGPADLVo9USmv5NVGUKyxnSmZFDQM9pRHCFEC3ZkuZ1l',
            'duration': 23.891,
            'view_count': int,
            'concurrent_view_count': int,
        },
        ...
        'info_dict': {
            ...
            'thumbnail': r're:^https?://.*',
            'uploader': 'Lela Evans',
            'uploader_id': 'pfbid0skBr3dTRbpKhyXZgW4nA82nEVcwUtdGd2Vk1RodSabUceumcqG4upVmwsndC7fmvl',
            'uploader_url': r're:^https?://.*',
        ...
        'info_dict': {
            ...
            'thumbnail': r're:^https?://.*',
            'concurrent_view_count': int,
            'uploader_id': 'pfbid02gdQkVrbs5GSxdYggRmZWLZCstGCZV3tBsnAzxqLMNhdjDUHWVXMTaR6wNuNjU6kXl',
    ...
    def _extract_from_url(self, url, video_id):
                ...
                for pattern in (pattern_grp if isinstance(pattern_grp, tuple) else [pattern_grp]):  # return 1st match
                    ...
                    for i in (m.start() for m in re.finditer(pattern, json_string)):                # depend on get_all
                        ...
                        opening = (i + find_offset(
                            json_string[(i * obj_in_value):(i * (not obj_in_value) - obj_in_value)],
                            '{') - obj_in_value)
        ...
        data = titles = []
        for x in post_data:
            if not data and '"feed_unit":' not in x and '"dash_manifest_url":' in x:
                data = x
            if not titles and '"feed_unit":' not in x and '"title":{' in x and '"id":"' in x:
                titles = find_json_obj(x, r'"title":[^}]*"text":', get_all=True)
            if data and titles:
                break
        # print(data)
        # print(list(titles))
            ...
            if data:
                ...
                def parse_graphql_video(video):
                    ...
                    if uploader_id:
                        ...
                        ','.join(post_data), (r'[_"]creator":[^}]*"name":"', r'[_"]owner":[^}]*"name":"', r'"actor":[^}]*"name":"'), obj_in_value=True
                    )):
                    ...
                    info = {
                        'id': v_id,
                        'title': video.get('name') or title or f'Facebook video #{v_id}',
                ...
                if 'Facebook video #' in video_info['title'] and page_title:
