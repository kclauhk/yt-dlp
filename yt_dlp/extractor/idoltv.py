        ...
        if player_data['from'] == 'bilibili':
            ...
            if f := self._extract_m3u8_formats_and_subtitles(player_data['url'], video_id, errnote=None, fatal=False)[0]:
                ...
                if '.bfvvs.com' in f[0]['url'] or '.subokk.com' in f[0]['url']:
                    f[0]['format_note'] = f[0]['format_note'] + ', slow'
                                ...
                                if stream.get('codec_type') == 'video':
                                    ...
                                    f[0].update({
                                        **traverse_obj(stream, {
                                            ...
                                            # 'vcodec': ('codec_name', {lambda x: x.replace('h264', 'avc1') if x else None}),
                                        }),
    ...
    def _search_results(self, query):
        ...
        for page_number in itertools.count(1):
            ...
            if '<h1>404</h1>' in webpage:
                raise ExtractorError('Unable to download webpage: HTTP Error 404: Not Found (caused by <HTTPError 404: Not Found>)', expected=True)
            match_total = match_total or int_or_none(self._html_search_regex(r'\$\(.\.mac_total.\)\.html\(.(\d+).\);', webpage, 'mac_total', default=None))
            if match_total == 0:
                return
            ...
            for result in search_results:
                if path := self._search_regex(r'class="vodlist_title"><a href="(.+)" title="', result, 'link', default=None):
