...
class IdoltvIE(InfoExtractor):
    ...
    _TESTS = [{
        'url': 'https://idoltv.tv/play/552-2-190.html',
        'info_dict': {
            'id': '552-2-190',
            'ext': 'mp4',
            'title': 'Running Man | E726.241103',
        ...
        'url': 'https://idoltv.tv/play/1987-1-14.html',
        'info_dict': {
            'id': '1987-1-14',
            'ext': 'mp4',
            'title': '無謀挑戰 | 20050723',
            'description': 'md5:9fdfee338d82628de622099df0e67de6',
            'episode': '20050723',
            'thumbnail': r're:https?://.*',
            'cast': ['劉在錫'],
            'categories': ['韓綜'],
        ...
        'url': 'https://idoltv.tv/play/2209-2-12.html',
        'info_dict': {
            'id': '2209-2-12',
            'ext': 'mp4',
            'title': '我獨自李食堂 | 20201002 Ep10',
            'description': 'md5:e7831d651aba976a51a980a0fb845b3d',
            'episode': '20201002 Ep10',
            'thumbnail': r're:https?://.*',
            'cast': ['李壽根'],
            'categories': ['韓綜'],
            'tags': ['2020', '韓國', '搞笑', '美食', '體驗', '真人秀'],
        },
        'params': {'skip_download': True},
    }]

    _base_url = 'https://idoltv.tv'
    ...
    def _parse_episode(self, string):
        ...
        for e in episode.split(' '):
            ...
            if re.search(r'^(SD|HD|FHD|\d{3,4}P|標清|超清|高清|正片|中字)', e, re.IGNORECASE):
                x.append(('RES', part, 'r'))
            if len(x) == 0:
                x.append((e, part, 'n'))
            ep = ep + x
    ...
    def _extract_formats(self, video_id, episode_label, media_source, player_data):
        ...
        if player_data['from'] == 'bilibili':
            ...
            _skip_sources = ['.gggread.com', 'm3u8.hmrvideo.com', 'v8.tlkqc.com', 'hnzy.bfvvs.com',
                             '.fsvod1.com', 'hnzy3.hnzyww.com', 'cdn6.shzbgyl.com', 'cdn7.hbhaoyi.com']
            if all(x not in player_data['url'] for x in _skip_sources):
                formats = []
                for f in self._extract_m3u8_formats_and_subtitles(
                        player_data['url'], video_id, errnote=None, fatal=False)[0]:
                    f['format_id'] = self._html_search_regex(r'https?://[^/]*?\.?([\w]+)\.\w+/',
    ...
    def _real_extract(self, url):
        ...
        video_id = f'{vid}-{source_id}-{episode_id}'
        url = f'{self._base_url}/play/{vid}-{source_id}-{episode_id}.html'
        ...
        video_inf = re.findall(r'<h2 class="title margin_0">(.+)</h2>[\s\S]*<p class="nstem data ms_p margin_0">\s*(<span class[\s\S]+</span></span>)?\s*(<a href=.+</a>)[\s\S]*<div class="panel play_content.+>\s*<p>([\s\S]+)</p>\s*</div>[\s\S]*播放地址',
                               webpage)[0]
        title = (video_inf[0]
            ...
            for e in epsd:
                ...
                for y in x['links']:
                    ...
                    if y not in other_src and e in ep:
                        z.append(y)
                    if (e[2] == 'd' and abs(
                            (datetime.datetime.strptime(e[0], '%y%m%d')
        ...
        for x in other_src:
            self.to_screen(f'Extracting URL: {self._base_url}{x[0]}')
            page = (self._download_webpage(f'{self._base_url}{x[0]}', video_id)).replace('&nbsp;', ' ')
...
class IdoltvVodIE(IdoltvIE):
    ...
    def _real_extract(self, url):
        ...
        webpage = (self._download_webpage(f'{self._base_url}/vod/{video_id}.html', video_id)).replace('&nbsp;', ' ')
        if webpage.count('<h1>404</h1>'):
            raise ExtractorError(
                'Unable to download webpage: HTTP Error 404: Not Found (caused by <HTTPError 404: Not Found>)', expected=True)
        # extract video info
        fulltitle = (self._html_extract_title(webpage)
                     or self._html_search_meta(['og:title', 'twitter:title'], webpage, 'title'))
        video_inf = re.findall(
            r'<h2 class="title[\s\S]*itemprop="name">(.+)</span>[\s\S]*id="year">.*>(\d+)</a>[\s\S]*id="area">.*>(.+)</a>[\s\S]*id="class">.*>(.+)</a>', webpage)[0]
        title = (video_inf[0]
                 or fulltitle.split(' | ')[0])
        ...
        description = (clean_html(cast[1]) + ' \n' + clean_html(cast[0]) + ' \n'
                       + (self._html_search_regex(
                           r'<div class="content_desc full_text clearfix" id="description">([\s\S]+?)</span>',
        ...
        for i, x in enumerate(links):
            if x['source'] == 'bilibili':
                for y in x['links']:
                    ...
                    bili = idoltv._real_extract(f'{self._base_url}{y[0]}')
                    if 'entries' in bili:
        ...
        if playlist:
            entries = entries + [self.url_result(f'{self._base_url}{x[0]}', IdoltvIE)
    ...
    def _search_results(self, query):
        ...
        for page_number in itertools.count(1):
            webpage = self._download_webpage(
                f'https://idoltv.tv/vodsearch/page/{page_number}/wd/' + urllib.parse.unquote_plus(query) + '.html',
