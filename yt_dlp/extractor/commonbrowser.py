...
class BrowserIE(InfoExtractor):
    ...
    def _browser_get_webpage(
        ...
        request_timeout = self.get_param('socket_timeout') or 15
        ...
        if dp_available:
            ...
            if not self._browser or not self._browser.states.is_alive:
                ...
                # the path of the folder containing the turnstile patch
                # EXTENSION_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), 'turnstile'))
            ...
            while True:
                try:
                    res = tab.get(url, retry=1, timeout=request_timeout)
                    if (res if isinstance(res, bool) else res.get('ok')):
                        if '(function(){window._cf_chl_opt' not in tab.html:
                            return tab.html
                        else:
                            tab.run_js('try { turnstile.reset() } catch(e) { }')
                            turnstile_response = None
                            for _ in range(15 if try_count else 30):
                                try:
                                    turnstile_response = tab.run_js(
                    ...
                    raise ExtractorError(
                        f'{errnote}: {err}', cause=err, video_id=video_id, expected=True) from err
            ...
            async def download_webpage(url):
                if not self._browser or self._browser.stopped:
                    ...
                    except (ConnectionError, EOFError, Exception) as err:
                        raise ExtractorError(f'Failed to start browser: {err}') from err
                        ...
                        for _ in range(15 if try_count else 30):
                            if evaluate(content):
                                break
                                    ...
                                    r'var loadTimeDataRaw\s*=', content, 'loadtime',
                                    video_id, end_pattern=';</script', fatal=False,
                                    default=None):
                                msg = traverse_obj(data, ('heading', 'msg', {str}))
                                sub_msg = self._html_search_regex(
                                    r'main-message">[\s\S]+?<p>([^$].+)</p', content,
                                    'message', fatal=False, default=None)
                                if sub_msg == msg:
                                    sub_msg = None
                                raise TransportError(
                                    join_nonempty(f'{msg}.', sub_msg, delim=' '))
                            elif is_challenge(content) or self._new_response:
                                if '(function(){window._cf_chl_opt' in content:
                                    await asyncio.wait_for(page.verify_cf(), timeout=10)
                                self._new_response = False
                                self.to_screen(f'{video_id}: Waiting for 1 second')
                                await self._browser.wait(1)
                            content = await page.get_content()
    ...
    def _download_webpage(
        ...
        try:
            if self.ie_key() in self._browser_required:
                return self._browser_get_webpage(
                    url, video_id, note, errnote, fatal, 3, 0, browser_config)
            else:
                return super()._download_webpage(
                    url, video_id, note, errnote, fatal, tries, timeout, *args, **kwargs)
            ...
            if is_challenge(response):
                try:
                    self._browser_required.append(self.ie_key())
                    return self._browser_get_webpage(
                        url, video_id, note, errnote, fatal, 3, 0, browser_config)
                except BaseException as err:
                    if isinstance(err.cause, HTTPError) and err.cause.status == 403:
                        raise ExtractorError(
